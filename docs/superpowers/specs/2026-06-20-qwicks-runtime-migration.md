# QWicks Runtime - QWicks 功能迁移设计文档

**日期：** 2026-06-20  
**方案：** 渐进式迁移（方案 A）  
**目标：** 将 QWicks 项目的完整后端运行时功能迁移到 QWicks，适配 QWicks 架构，替换所有 QWicks 相关命名
...
QWicks 当前仅有 `qwicks/src/contracts` 目录，缺少以下核心模块：
...
| 原名称（QWicks） | 新名称（QWicks） |
|--------------|-------------------|
| `qwicks` | `qwicks` |
| `QWicks` | `QWicks` |
| `QWICKS` | `QWICKS` |
| `~/.deepseekgui/qwicks` | `~/.qwicksgui/qwicks` |
...
| 原变量 | 新变量 |
|--------|--------|
| `QWICKS_CONFIG` | `QWICKS_CONFIG` |
| `QWICKS_HOST` | `QWICKS_HOST` |
| `QWICKS_PORT` | `QWICKS_PORT` |
| `QWICKS_DATA_DIR` | `QWICKS_DATA_DIR` |
| `QWICKS_RUNTIME_TOKEN` | `QWICKS_RUNTIME_TOKEN` |
| `QWICKS_BASE_URL` | `QWICKS_BASE_URL` |
| `QWICKS_MODEL` | `QWICKS_MODEL` |
...
|--------|------|------|
| 默认端口 | 8899 | 8898 |
| 数据目录 | `~/.deepseekgui/qwicks` | `~/.qwicksgui/qwicks` |
...
**当前状态：** `qwicks/src/contracts` 已存在但内容不完整
...
**需要迁移的文件：**
- `qwicks-config.ts` - 运行时配置
- `secret-redaction.ts` - 密钥脱敏

**配置结构：**

```typescript
interface QWicksAgentConfig {
  serve: {
    host: string           // 默认 '127.0.0.1'
    port: number           // 默认 8898
    dataDir: string        // 默认 '~/.qwicksgui/qwicks'
    runtimeToken: string
    apiKey: string
    baseUrl: string
    model: string
...
- [ ] config 层所有文件迁移完成
- [ ] cache 层所有文件迁移完成
- [ ] 所有文件中无 `qwicks`、`QWicks`、`QWICKS`、`deepseekgui` 字样（除模型相关）
- [ ] TypeScript 编译通过
- [ ] 单元测试通过
- [ ] 导出接口与前端兼容
