...
release_load_local_env() {
  local env_file="${QWICKS_RELEASE_ENV:-${DEEPSEEK_GUI_RELEASE_ENV:-}}"
...
release_compute_version() {
  ...
  if [[ "${RELEASE_BUMP:-}" == "none" && -n "${RELEASE_TAG:-}" ]]; then
    ...
    RELEASE_NAME="QWicks ${RELEASE_VERSION}"
    LATEST_TAG=""
    return
  fi
...
release_clean_dist_artifacts() {
  ...
  rm -f "${ROOT}"/dist/QWicks-* "${ROOT}"/dist/DeepSeek-GUI-* "${ROOT}"/dist/DeepSeek\ GUI-* "${ROOT}"/dist/latest*.yml "${ROOT}"/dist/*.blockmap
}
