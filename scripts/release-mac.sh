...
#   QWICKS_RUNTIME_CACHE=0             disable bundled runtime cache
#
...
copy_mac_arch_artifacts() {
  ...
  shopt -s nullglob
  files=("${output_dir}"/QWicks-*-mac-"${arch}".*)
  shopt -u nullglob
...
# artifactName: QWicks-${version}-mac-${arch}.dmg|zip
collect "macOS arm64 dmg" "dist/QWicks-*-mac-arm64.dmg"
collect "macOS x64 dmg" "dist/QWicks-*-mac-x64.dmg"
collect "macOS arm64 zip" "dist/QWicks-*-mac-arm64.zip"
collect "macOS x64 zip" "dist/QWicks-*-mac-x64.zip"
collect_optional "macOS blockmap" "dist/QWicks-*-mac-*.zip.blockmap"
...
cyan "  https://github.com/QWicksAgent/QWicks/releases/tag/${TAG_NAME}"
