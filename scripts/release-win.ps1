...
function Load-LocalReleaseEnv([string]$RootPath) {
  $configured = [Environment]::GetEnvironmentVariable('QWICKS_RELEASE_ENV', 'Process')
  if (-not $configured) {
...
$RequestedChannel = if ($Stable) {
  ...
  $env:RELEASE_CHANNEL
} elseif ($env:QWICKS_UPDATE_CHANNEL) {
  $env:QWICKS_UPDATE_CHANNEL
