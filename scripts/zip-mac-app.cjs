...
const version = (process.env.QWICKS_APP_VERSION || process.env.DEEPSEEK_GUI_APP_VERSION || pkg.version || '').trim()
if (!version) {
  console.error('[zip-mac-app] Could not resolve package version.')
  process.exit(1)
}

const distDir = resolve(process.env.QWICKS_DIST_DIR || process.env.DEEPSEEK_GUI_DIST_DIR || join(root, 'dist'))
const appOutDir = join(distDir, arch === 'arm64' ? 'mac-arm64' : 'mac')
const appName = 'QWicks.app'
const appPath = join(appOutDir, appName)
const zipPath = join(distDir, `QWicks-${version}-mac-${arch}.zip`)
