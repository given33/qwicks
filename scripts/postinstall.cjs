...
require('./ensure-qwicks-install.cjs')

const buildQWicks = run('npm', ['--prefix', 'qwicks', 'run', 'build'])
if (buildQWicks.status !== 0) {
  process.exit(buildQWicks.status || 1)
}

// QWicks is spawned with the Electron binary (ELECTRON_RUN_AS_NODE) and resolves
...
// loaded there and QWicks would silently fall back to JSONL scanning. Best
...
try {
  ...
  if (result.status !== 0) {
    console.warn('[postinstall] better-sqlite3 electron prebuild failed; QWicks will use the JSONL fallback')
  }
} catch (error) {
