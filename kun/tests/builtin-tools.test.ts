...
async function createFakeFdExecutable(workspace: string): Promise<string> {
  const toolDir = join(workspace, 'tools')
  await mkdir(toolDir, { recursive: true })
  if (process.platform === 'win32') {
    const scriptPath = join(toolDir, 'fd-fake.cmd')
    await writeFile(
      scriptPath,
      [
        '@echo off',
  ...
  const scriptPath = join(toolDir, 'fd-fake')
  await writeFile(
    scriptPath,
    [
      '#!/bin/sh',
...
function backgroundChildCommand(): string {
  const script = [
    "const { spawn } = require('node:child_process');",
    "const child = spawn(process.execPath, ['-e', 'setTimeout(() => {}, 5000)'], { cwd: require('node:os').tmpdir(), detached: true, stdio: 'inherit' });",
    'child.unref();',
    "console.log('done');"
  ].join(' ')
...
describe('Teamflow Agent built-in tools', () => {
  let workspace: string
  let host: LocalToolHost

  beforeEach(async () => {
    workspace = await mkdtemp(join(tmpdir(), 'teamflow-agent-tools-'))
  ...
  it('prefers the fd backend path when an fd executable candidate is provided', async () => {
    ...
    const fakeFd = await createFakeFdExecutable(workspace)
    const fdHost = new LocalToolHost({
      tools: [
        createFindLocalTool({
          fdExecutableCandidates: [fakeFd],
  ...
  it('writes, reads, edits, and searches workspace files', async () => {
    ...
    const editOutput = await executeTool(host, workspace, 'edit', {
      path: 'notes/demo.txt',
      edits: [
        { oldText: 'hello world', newText: 'hello teamflow-agent' },
    ...
    expect(editedDisk).toContain('hello teamflow-agent')
    expect(editedDisk).toContain('done')
    expect(String(editOutput.diff)).toContain('+2 hello teamflow-agent')
  ...
  it('finishes bash commands after the shell exits even when a background child keeps stdio open', async () => {
    ...
    const output = await executeTool(host, workspace, 'bash', {
      command: backgroundChildCommand(),
      timeout: 2
    })
