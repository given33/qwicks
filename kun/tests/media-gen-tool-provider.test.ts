...
import { TeamflowAgentCapabilitiesConfig } from '../src/contracts/capabilities.js'
...
describe('Media gen tool provider', () => {
  beforeEach(async () => {
    workspace = await mkdtemp(join(tmpdir(), 'teamflow-agent-mediagen-'))
  })
  ...
  it('generates speech, music, and video files through configured media tools', async () => {
    ...
    await expectFile(speechOutput, '.teamflowgui-audio/', 'audio/mpeg', 'speech-bytes')
    await expectFile(musicOutput, '.teamflowgui-music/', 'audio/mpeg', 'music-bytes')
    await expectFile(videoOutput, '.teamflowgui-videos/', 'video/mp4', 'video-bytes')
