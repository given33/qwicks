...
describe('Teamflow Agent CLI commands', () => {
  let dataDir = ''

  beforeEach(async () => {
    dataDir = await mkdtemp(join(tmpdir(), 'teamflow-agent-cli-'))
  })
  ...
  it('splits explicit commands and keeps legacy serve flags compatible', () => {
    expect(splitTeamflowAgentCliCommand(['run', 'hello'])).toEqual({ command: 'run', args: ['hello'] })
    expect(splitTeamflowAgentCliCommand(['--port', '9999'])).toEqual({
      command: 'serve',
      args: ['--port', '9999']
    })
    expect(splitTeamflowAgentCliCommand(['nope']).error).toMatch(/unknown command/)
  })

  it('lists tools from teamflow-agent exec with JSON output', async () => {
    const c = capture()
  ...
  it('invokes a direct tool through teamflow-agent exec', async () => {
    const c = capture()
    const code = await runAgentCommand('exec', [
      '--data-dir',
      dataDir,
  ...
  it('lists dynamic runtime tools from teamflow-agent exec', async () => {
  ...
  it('shares config loading between serve and agent commands', async () => {
    const configPath = join(dataDir, 'teamflow-agent.config.json')
