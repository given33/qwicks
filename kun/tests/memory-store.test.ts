...
import { TeamflowAgentCapabilitiesConfig, type MemoryCapabilityConfig } from '../src/contracts/capabilities.js'
...
describe('Memory store and recall', () => {
  ...
  beforeEach(async () => {
    dir = await mkdtemp(join(tmpdir(), 'teamflow-agent-memory-'))
    nextId = 1
  })
