...
function imageGenConfig(overrides: Record<string, unknown> = {}) {
  return TeamflowAgentCapabilitiesConfig.parse({
    imageGen: {
      enabled: true,
...
function attachmentStore(rootDir: string, overrides: Record<string, unknown> = {}) {
  return new FileAttachmentStore({
    rootDir,
    config: TeamflowAgentCapabilitiesConfig.parse({ attachments: { enabled: true, ...overrides } }).attachments,
...
describe('Image gen tool provider', () => {
  beforeEach(async () => {
    workspace = await mkdtemp(join(tmpdir(), 'teamflow-agent-imagegen-'))
  })
  ...
  it('generates an image, saves it to the workspace, and scopes the attachment', async () => {
    ...
    expect(output.files[0].relativePath.startsWith('.teamflowgui-images/')).toBe(true)
