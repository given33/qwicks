...
import { DEFAULT_TEAMFLOW_AGENT_MODEL } from '../src/config/teamflow-agent-config.js'
...
describe('ThreadService todos', () => {
  it('syncs plan checklists, patches linked checkboxes, and preserves removed tasks', async () => {
    const workspace = await mkdtemp(join(tmpdir(), 'teamflow-agent-todos-'))
    try {
...
describe('ThreadService.resumeSession', () => {
  it('uses the Teamflow Agent default model when resuming item-only legacy sessions', async () => {
    const { service, sessionStore } = buildService()
    ...
    expect(result.thread.model).toBe(DEFAULT_TEAMFLOW_AGENT_MODEL)
  })
})
