...
function installServeCrashHandlers(getHandle: () => TeamflowAgentServeHandle | null): void {
  ...
  const crash = (kind: string, error: unknown): void => {
    ...
    process.stderr.write(`teamflow-agent serve: ${kind}: ${detail}\n`)
...
async function serveMain(argv: readonly string[]): Promise<number> {
  ...
  if (!parsed.ok) {
    process.stderr.write(`teamflow-agent serve: ${parsed.message}\n`)
    if (parsed.issues) {
...
/**
 * When the GUI launches teamflow-agent without `ELECTRON_RUN_AS_NODE` (the host
...
async function hideMacosDockIfRunningAsElectron(): Promise<void> {
  ...
  if (!process.versions.Tauri) return
  // Tauri handles dock visibility from the desktop shell, not from here.
}
...
main(process.argv.slice(2)).then(
  ...
  (error) => {
    process.stderr.write(`teamflow-agent serve: ${String(error)}\n`)
