...
import {
  teamflowAgentConfigPathForDataDir,
  readTeamflowAgentConfigFile,
  readOptionalTeamflowAgentConfigFile,
  type LoadedTeamflowAgentConfig
} from '../config/teamflow-agent-config.js'

/**
 * Parse the `teamflow-agent serve` command line into validated options.
 *
...
export function parseServeOptions(
  ...
  const merged: ServeOptions = {
    ...
    hooks: loadedConfig?.config.hooks as never,
...
export const SERVE_USAGE = `teamflow-agent serve [options]
...
function loadServeConfig(
  ...
  env: Record<string, string | undefined>
): LoadedTeamflowAgentConfig | null {
  ...
  return readOptionalTeamflowAgentConfigFile(teamflowAgentConfigPathForDataDir(dataDir))
}
