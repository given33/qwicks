...
export const TurnSchema = z.object({
   ...
   * headless runs). Teamflow Agent hides `user_input`/`request_user_input` and
...
export const StartTurnRequest = z.object({
   ...
   * mode Teamflow Agent advertises `create_plan` for the whole conversation.
   */
  mode: TurnModeSchema.optional(),
  attachments: z
    .array(
  ...
  workspaceCheckpointId: z.string().min(1).optional(),
  /**
   * Optional GUI plan context. When set, Teamflow Agent advertises the
