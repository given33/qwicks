...
import {
  ...
  type TeamflowAgentCapabilitiesConfig
...
import {
  ...
  type StorageConfig
} from '../config/teamflow-agent-config.js'
...
import { TEAMFLOW_AGENT_SYSTEM_PROMPT } from '../prompt/teamflow-agent-system-prompt.js'
...
export type TeamflowAgentServeRuntimeOptions = {
  ...
  capabilities?: TeamflowAgentCapabilitiesConfig
...
export async function createTeamflowAgentServeRuntime(
  ...
  let prefix = createImmutablePrefix({
    systemPrompt: TEAMFLOW_AGENT_SYSTEM_PROMPT,
    pinnedConstraints: [
  ...
  if (skillCatalog) {
    prefix = setSystemPrompt(prefix, `${TEAMFLOW_AGENT_SYSTEM_PROMPT}\n\n${skillCatalog}`)
  }
  ...
  void runtime.turnService
    ...
    .then(async (threadIds) => {
      if (threadIds.length > 0) {
        console.warn(`[teamflow-agent] marked orphaned turn(s) on ${threadIds.length} thread(s) as failed after restart`)
      }
      if (threadIds.length > 0 && runtime.resumeInterruptedGoals) {
        const resumed = await runtime.resumeInterruptedGoals(threadIds)
        if (resumed > 0) {
          console.warn(`[teamflow-agent] auto-resumed ${resumed} interrupted goal(s) after restart`)
        }
      }
    })
    .catch((error) => {
      console.warn('[teamflow-agent] orphaned turn reconciliation failed:', error)
    })
  return {
    ...server,
    runtime,
