...
const HOME_DIR_ARG = '--qwicks-home-dir='
...
const api = {
  ...
  detectLegacySessions: () =>
    ipcRenderer.invoke('qwicks:sessions:detect-legacy'),
  importLegacySessions: (sourceDir) =>
    ipcRenderer.invoke('qwicks:sessions:import-legacy', { sourceDir }),
  pickLegacySessionDir: () =>
    ipcRenderer.invoke('qwicks:sessions:pick-source-dir'),
  ...
  getQWicksConfigFile: () =>
    ipcRenderer.invoke('qwicks:config:read'),
  setQWicksConfigFile: (content) =>
    ipcRenderer.invoke('qwicks:config:write', content),
  openQWicksConfigDir: () =>
    ipcRenderer.invoke('qwicks:config:open-dir'),
...
} satisfies QWicksGuiApi

contextBridge.exposeInMainWorld('qwicksGui', api)
