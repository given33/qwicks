...
// The generate-image node lazily imports the qwicks image client. Replace it with a
// stub so the test never hits a real provider (and never pulls native deps in).
vi.mock('../../qwicks/src/adapters/tool/image-gen-tool-provider.js', () => ({
...
function buildSettings(
  ...
  const base: AppSettingsV1 = {
    ...
    agents: { qwicks: { ...defaultQWicksRuntimeSettings(), model: 'test-model', apiKey: 'test-key' } },
...
/** The prompt the AI node actually sent to the qwicks runtime (from the /turns request). */
...
describe('ai-agent', () => {
  it('runs the prompt through the qwicks runtime and returns the reply', async () => {
    cover('ai-agent')
  ...
  it('interpolates {{ }} from the upstream payload into the prompt', async () => {
    ...
    expect(turnPrompt(rr)).toBe('echo QWicks')
  }, 15_000)
  ...
  it('appends the upstream input to the prompt when it uses no {{ }}', async () => {
    ...
    expect(prompt).toContain('QWicks')
  }, 15_000)
          ...
          patch: (s) => ({
            ...
            agents: {
              qwicks: {
                ...s.agents.qwicks,
                imageGeneration: {
                  ...s.agents.qwicks.imageGeneration,
                  enabled: true,
                  providerId: '',
...
describe('output', () => {
  ...
  it('drills into a json path (json mode)', async () => {
    const result = await testNode({ id: 'o', type: 'output', config: { mode: 'json', jsonPath: 'user.name' } }, '{"user":{"name":"QWicks"}}')
    expect(parseOut(result)).toBe('QWicks')
  }, 15_000)

  it('coerces a missing json path to null (json mode)', async () => {
    const result = await testNode({ id: 'o', type: 'output', config: { mode: 'json', jsonPath: 'user.missing.deep' } }, '{"user":{"name":"QWicks"}}')
    expect(result.status).toBe('success')
...
describe('custom', () => {
  it('runs a custom module with its injected $fields', async () => {
    ...
    const result = await testNode({ id: 'c', type: 'custom', config: { moduleId: 'mod-greet', values: { who: 'QWicks' } } }, '{}', {
      modules: [module]
    })
    expect(result.status).toBe('success')
    expect(parseOut(result)).toEqual({ greeting: 'hi QWicks' })
  }, 15_000)
