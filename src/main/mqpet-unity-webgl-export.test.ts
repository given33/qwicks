...
describe('export-unity-webgl', () => {
  ...
  it('defaults the export output to the QWicks userData Unity WebGL directory', async () => {
    const modulePath = new URL('../../tools/mqpet-source/export-unity-webgl.mjs', import.meta.url).href;
    const { exportUnityWebGL } = await import(`${modulePath}?default-output-${Date.now()}`) as ExportModule;
    const projectRoot = join(tmpdir(), `qwicks-mqpet-export-default-${process.pid}-${Date.now()}`);
    const appDataRoot = join(projectRoot, 'AppData', 'Roaming');
    const outputDir = join(appDataRoot, 'QWicks', 'mqpet', 'unity-webgl');
    const unityEditor = join(projectRoot, 'Unity.exe');
    const previousEnvOutput = process.env.QWICKS_MQPET_UNITY_WEBGL_DIR;
    ...
    try {
      delete process.env.QWICKS_MQPET_UNITY_WEBGL_DIR;
      ...
      if (previousEnvOutput === undefined) delete process.env.QWICKS_MQPET_UNITY_WEBGL_DIR;
      else process.env.QWICKS_MQPET_UNITY_WEBGL_DIR = previousEnvOutput;
