...
import { QWicksConfigSchema } from '../../qwicks/src/config/qwicks-config.js'
...
function createSettings(binaryPath: string): AppSettingsV1 {
  return {
    ...
    agents: {
      qwicks: {
        ...defaultQWicksRuntimeSettings(8899),
        binaryPath,
        autoStart: true
      }
    },
...
async function readQWicksLog(): Promise<string> {
  if (!tempRoot) throw new Error('temp root not initialized')
  for (let attempt = 0; attempt < 40; attempt += 1) {
    const logFile = readdirSync(tempRoot).find((entry) => entry.startsWith('qwicks-') && entry.endsWith('.log'))
    if (logFile) return readFileSync(join(tempRoot, logFile), 'utf8')
    await new Promise((resolve) => setTimeout(resolve, 25))
  }
  throw new Error('Expected a qwicks log file to be created')
}
...
beforeEach(() => {
  tempRoot = mkdtempSync(join(tmpdir(), 'qwicks-process-'))
  configureLogger({ dir: tempRoot, enabled: true, retentionDays: 7 })
})

afterEach(async () => {
  const module = await import('./qwicks-process')
  await module.stopQWicksChildAndWait()
...
describe('startQWicksChild', () => {
  it('waits for the explicit QWicks ready marker before resolving', async () => {
    const script = writeScript(
      'ready-child.js',
      [
        "setTimeout(() => {",
        "  process.stdout.write('QWICKS_READY ' + JSON.stringify({ service: 'qwicks', mode: 'serve', port: 8899 }) + '\\n')",
        "}, 50)",
        "setInterval(() => {}, 1_000)"
      ].join('\n')
    )
    const module = await import('./qwicks-process')
    await expect(module.startQWicksChild(createSettings(script))).resolves.toBeUndefined()
    expect(module.isQWicksChildRunning()).toBe(true)
    await module.stopQWicksChildAndWait()
    const logText = await readQWicksLog()
    expect(logText).toContain('QWICKS_READY')
    expect(logText).toContain('ready marker received on port 8899')
  })

  it('shares the startup promise while QWicks is spawned but not ready', async () => {
    if (!tempRoot) throw new Error('temp root not initialized')
    ...
    const first = module.startQWicksChild(settings)

    for (let attempt = 0; attempt < 100 && !module.isQWicksChildRunning(); attempt += 1) {
      await new Promise((resolve) => setTimeout(resolve, 10))
    }
    expect(module.isQWicksChildRunning()).toBe(true)

    let secondResolved = false
    const second = module.startQWicksChild(settings).then(() => {
      secondResolved = true
    })
  ...
  it('rejects when the child exits before reporting ready', async () => {
    ...
    const module = await import('./qwicks-process')
    await expect(module.startQWicksChild(createSettings(script))).rejects.toThrow(
      /QWicks exited during startup with code 23[\s\S]*bind failed on port 8899/
    )
    expect(module.isQWicksChildRunning()).toBe(false)
    await module.stopQWicksChildAndWait()
    const logText = await readQWicksLog()
...
describe('reclaimQWicksPort', () => {
  it('reports a port as unavailable when another listener owns it', async () => {
    ...
    try {
      ...
      await expect(module.reclaimQWicksPort(address.port)).resolves.toEqual({
        ok: false,
  ...
  it('allows non-positive ports so QWicks can request an ephemeral port', async () => {
    const module = await import('./qwicks-process')

    await expect(module.reclaimQWicksPort(0)).resolves.toEqual({ ok: true })
  })
  ...
  it('resolves the next available fallback port when the preferred port is unavailable', async () => {
    ...
    try {
      ...
      const resolved = await module.resolveAvailableQWicksPort(preferredPort)
      ...
      await expect(module.reclaimQWicksPort(resolved.port)).resolves.toEqual({ ok: true })
    } finally {
      await new Promise<void>((resolve) => server.close(() => resolve()))
    }
  })
...
describe('resolveQWicksDataDir', () => {
  it('expands Windows-style home-relative data directories', async () => {
    const module = await import('./qwicks-process')

    expect(module.resolveQWicksDataDir({ dataDir: '~\\deepseek\\qwicks' })).toBe(join(homedir(), 'deepseek', 'qwicks'))
  })

  it('does not expand non-home tilde prefixes', async () => {
    const module = await import('./qwicks-process')

    expect(module.resolveQWicksDataDir({ dataDir: '~other\\qwicks' })).toBe('~other\\qwicks')
  })
})
...
describe('parseListeningPidsFromNetstat', () => {
  it('extracts the listening TCP PIDs for the port across IPv4/IPv6, ignoring everything else', async () => {
    const { parseListeningPidsFromNetstat } = await import('./qwicks-process')
    const output = [
      '',
      'Active Connections',
      '',
...
describe('syncGuiManagedQWicksConfig', () => {
  it('creates GUI-managed config with attachments enabled for image paste/upload', async () => {
    if (!tempRoot) throw new Error('temp root not initialized')
    const configPath = join(tempRoot, 'config.json')
    const module = await import('./qwicks-process')

    await module.syncGuiManagedQWicksConfig(tempRoot, defaultQWicksRuntimeSettings())
  ...
  it('writes the memory capability from the GUI memory toggle', async () => {
    ...
    await module.syncGuiManagedQWicksConfig(tempRoot, {
      ...defaultQWicksRuntimeSettings(),
      memoryEnabled: true
    })
  ...
  it('writes the image generation capability and omits cleared fields', async () => {
    ...
    expect(QWicksConfigSchema.safeParse(parsed).success).toBe(true)
  ...
  it('adds the built-in schedule MCP server to QWicks runtime capabilities', async () => {
    if (!tempRoot) throw new Error('temp root not initialized')
    const configPath = join(tempRoot, 'config.json')
    const module = await import('./qwicks-process')
    const settings = createSettings('/tmp/fake-qwicks-child.js')
  ...
  it('adds GUI project and configured global skill roots to QWicks runtime capabilities', async () => {
    if (!tempRoot) throw new Error('temp root not initialized')
    const configPath = join(tempRoot, 'config.json')
    const module = await import('./qwicks-process')
    const settings = createSettings('/tmp/fake-qwicks-child.js')
  ...
  it('writes GUI-managed MCP search settings without removing existing servers', async () => {
    ...
    await module.syncGuiManagedQWicksConfig(tempRoot, {
      ...
      storage: {
        backend: 'hybrid',
        sqlitePath: '/tmp/qwicks-index.sqlite3'
      },
      contextCompaction: {
  ...
  it('replaces unparsable historical QWicks config with a valid GUI-managed config', async () => {
    if (!tempRoot) throw new Error('temp root not initialized')
    const configPath = join(tempRoot, 'config.json')
  ...
  it('does not enable MCP when the capability is explicitly disabled', async () => {
    ...
    await module.syncGuiManagedQWicksConfig(tempRoot, defaultQWicksRuntimeSettings(), {
      scheduleMcp: {
        settings: createSettings('/tmp/fake-qwicks-child.js'),
        launch: {
          appPath: '/tmp/deepseek-gui-test-app',
          execPath: '/tmp/electron',
          isPackaged: false
