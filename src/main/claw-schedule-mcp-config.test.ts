...
const launch: ClawScheduleMcpLaunchConfig = {
  appPath: '/Applications/QWicks.app',
  execPath: '/Applications/QWicks.app/Contents/MacOS/QWicks',
  isPackaged: false
}

describe('claw schedule MCP config', () => {
  it('uses QWicks config files by default', () => {
    expect(resolveQWicksConfigPath()).toBe(join(homedir(), '.qwicks', 'config.toml'))
    expect(resolveQWicksMcpJsonPath()).toBe(join(homedir(), '.qwicks', 'mcp.json'))
    expect(resolveDeepseekConfigPath()).toBe(resolveQWicksConfigPath())
  })

  it('writes the gui_schedule server to the QWicks MCP JSON config shape', () => {
  ...
  it('uses the macOS Electron helper for real app bundle paths', () => {
    expect(resolveClawScheduleMcpCommand(launch, 'darwin')).toBe(
      '/Applications/QWicks.app/Contents/Frameworks/QWicks Helper.app/Contents/MacOS/QWicks Helper'
    )
  ...
  it('syncs mcp.json and cleans the old config.toml entry on disk', async () => {
    ...
    const configTomlPath = join(qwicksDir, 'config.toml')
    const mcpJsonPath = join(qwicksDir, 'mcp.json')
    await mkdir(qwicksDir, { recursive: true })
    await writeFile(
      configTomlPath,
      [
        'provider = "deepseek"',
