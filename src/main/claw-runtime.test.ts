...
describe('ClawRuntime', () => {
  ...
  it('reads assistant text from the QWicks thread detail shape used by the real runtime', async () => {
    const settings = buildSettings()
    const runtimeRequest = vi.fn(async (_settings, path, init) => {
      if (path === '/v1/threads') {
        return { ok: true, status: 200, body: JSON.stringify({ id: 'thr_1' }) }
  ...
  it('uses the current IM provider when starting an agent turn', async () => {
    ...
    const runtimeRequest = vi.fn(async (requestSettings: AppSettingsV1, path, init) => {
      expect(requestSettings.agents.qwicks.providerId).toBe('minimax')
      expect(requestSettings.agents.qwicks.model).toBe('MiniMax-M3')
  ...
  it('resolves the default IM auto model to the current QWicks model before starting a turn', async () => {
    const settings = buildSettings()
    settings.claw.im.enabled = true
    settings.claw.im.responseTimeoutMs = 2_000
    settings.agents.qwicks.providerId = 'xiaomi'
    settings.agents.qwicks.model = 'mimo-v2.5'
    settings.provider.providers = [
      ...settings.provider.providers,
      buildModelProvider({
        id: 'xiaomi',
    ...
    const runtimeRequest = vi.fn(async (requestSettings: AppSettingsV1, path, init) => {
      expect(requestSettings.agents.qwicks.providerId).toBe('xiaomi')
      expect(requestSettings.agents.qwicks.model).toBe('mimo-v2.5')
  ...
  it('handles webhook /help as an IM command before starting a QWicks turn', async () => {
    const settings = buildSettings()
    settings.claw.im.enabled = true
  ...
  it('sends the channel intro before handling the first Feishu message', async () => {
    ...
    expect(welcomeCall[1].markdown).toContain('QWicks')
  ...
  it('sends generated image tool output to Feishu for image requests', async () => {
    ...
    try {
      ...
      settings.claw.im.enabled = true
      settings.claw.im.responseTimeoutMs = 2_000
      settings.agents.qwicks.imageGeneration = {
        enabled: true,
        providerId: '',
  ...
  it('returns current-turn generated music files in the WeChat webhook reply for follow-up prompts', async () => {
    ...
    try {
      ...
      settings.claw.im.enabled = true
      settings.claw.im.responseTimeoutMs = 2_000
      settings.agents.qwicks.musicGeneration = {
        enabled: true,
        providerId: '',
  ...
  it('returns generated speech files in the WeChat webhook reply for voice requests', async () => {
    ...
    try {
      ...
      settings.claw.im.enabled = true
      settings.claw.im.responseTimeoutMs = 2_000
      settings.agents.qwicks.textToSpeech = {
        enabled: true,
        providerId: '',
  ...
  it('returns current-turn generated video files in the WeChat webhook reply for follow-up prompts', async () => {
    ...
    try {
      ...
      settings.claw.im.enabled = true
      settings.claw.im.responseTimeoutMs = 2_000
      settings.agents.qwicks.videoGeneration = {
        enabled: true,
        providerId: '',
