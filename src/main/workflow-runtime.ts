...
import { resolveQWicksImageGenerationSettings } from '../shared/app-settings'
...
type NodeOutcome = {
  ...
  branch?: string
  /** For ai-agent nodes: the QWicks thread created. */
  threadId?: string
}
...
function resolveWorkflowImageGen(
  ...
  nodeModel: string
): ReturnType<typeof resolveQWicksImageGenerationSettings> {
  const providerId = nodeProviderId.trim()
  const model = nodeModel.trim()
  if (!providerId && !model) return resolveQWicksImageGenerationSettings(settings)
  const qwicks = settings.agents.qwicks
  ...
  return resolveQWicksImageGenerationSettings(patched)
}
...
export class WorkflowRuntime {
  ...
  private async handleWebhookRequest(req: IncomingMessage, res: ServerResponse): Promise<void> {
    try {
      ...
      if (secret) {
        const rawHeader = req.headers['x-qwicks-secret']
  ...
  private async handleInternalRequest(
    ...
    if (pathname === '/workflow/internal/hook-run') {
      // The hook payload (the qwicks invocation) is the workflow input; nodes read it via {{json.*}}.
  ...
  /** Run a workflow on behalf of the QWicks agent tool: resolve by id/name, await it, return its output. */
  ...
  /**
   * Run a workflow triggered by a qwicks agent hook. Resolves by id (no callableByAgent
  ...
  private async executeNode(
    ...
    switch (node.type) {
      ...
      case 'ai-agent': {
        const modelConfig = resolveScheduleModelConfig(
          settings,
          {
            ...
            model: node.config.model.trim() || settings.agents.qwicks.model,
      ...
      case 'generate-image': {
        ...
        // Lazy import keeps the qwicks image module out of the unit-test graph.
        const { createImageGenClient } = await import('../../qwicks/src/adapters/tool/image-gen-tool-provider.js')
      ...
      case 'parameter-extractor': {
        const modelConfig = resolveScheduleModelConfig(
          settings,
          { providerId: node.config.providerId, model: node.config.model.trim() || settings.agents.qwicks.model, reasoningEffort: node.config.reasoningEffort },
          settings.workflow.providerId?.trim() || ''
        )
        const workspace =
