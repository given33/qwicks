...
export type RunPromptOptions = {
  ...
  model: string
  /** Optional provider id; routed via QWicks's MultiProviderModelClient. */
  providerId?: string
...
// node runs a prompt through the exact same QWicks-runtime path as a scheduled
...
export type RunPromptViaRuntimeOptions = {
  ...
  model: string
  /**
   * Optional provider id override. Forwarded to QWicks's `POST /v1/threads` so
   * the thread (and every turn on it) routes its model request through the
   * qwicks-config `serve.providers[providerId]` HTTP client. Omitted or
...
export async function runPromptViaRuntime(
  ...
  const model = normalizeTaskModel(options.model) ?? (settings.agents.qwicks.model.trim() || DEFAULT_SCHEDULE_MODEL)
