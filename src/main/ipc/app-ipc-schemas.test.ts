...
describe('app-ipc-schemas', () => {
  ...
  it('accepts the QWicks runtime info endpoint', () => {
    const payload = runtimeRequestPayloadSchema.parse({
  ...
  it('accepts the QWicks runtime tool diagnostics endpoint', () => {
    const payload = runtimeRequestPayloadSchema.parse({
  ...
  it('accepts the QWicks skills endpoint', () => {
    const payload = runtimeRequestPayloadSchema.parse({
      path: '/v1/skills',
      method: 'GET'
    })
  ...
  it('accepts QWicks attachment and memory endpoints', () => {
  ...
  it('accepts QWicks thread goal endpoints', () => {
    expect(runtimeRequestPayloadSchema.parse({
  ...
  it('accepts the QWicks thread review endpoint', () => {
    expect(runtimeRequestPayloadSchema.parse({
  ...
  it('rejects runtime request paths outside the modeled QWicks API surface', () => {
    expect(() =>
  ...
  it('accepts a valid settings patch for qwicks and write settings', () => {
    ...
    expect(payload.agents?.qwicks?.port).toBe(9000)
    expect(payload.agents?.qwicks?.modelProfiles?.['custom-vision-model']?.inputModalities).toEqual(['text', 'image'])
    expect(payload.agents?.qwicks?.tokenEconomy?.enabled).toBe(true)
    expect(payload.agents?.qwicks?.tokenEconomy?.historyHygiene?.maxToolResultTokens).toBe(4000)
  ...
  it('accepts media generation settings and provider capability patches', () => {
    ...
    expect(payload.agents?.qwicks?.textToSpeech?.enabled).toBe(true)
    expect(payload.agents?.qwicks?.musicGeneration?.model).toBe('music-2.6')
    expect(payload.agents?.qwicks?.videoGeneration?.defaultResolution).toBe('1080P')
  })
  ...
  it('strips legacy settings keys while preserving current skill settings', () => {
    ...
    expect(payload.agents?.qwicks?.port).toBe(9001)
    expect(payload.agents?.qwicks?.imageRecognition).toEqual({ enabled: true })
  ...
  it('accepts a configurable stream idle timeout in runtime tuning patches', () => {
    ...
    expect(payload.agents?.qwicks?.runtimeTuning?.streamIdleTimeoutMs).toBe(300000)
  })

  it('rejects an out-of-range stream idle timeout', () => {
    expect(() =>
      settingsPatchSchema.parse({
        agents: { qwicks: { runtimeTuning: { streamIdleTimeoutMs: -1 } } }
      })
    ).toThrow()
  })
