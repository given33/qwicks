...
import { DEFAULT_QWICKS_DATA_DIR, resolveQWicksRuntimeSettings } from '../../shared/app-settings'
...
type RegisterAppIpcHandlersOptions = {
  ...
  resolveQWicksConfigPath: () => string
  onQWicksMcpConfigWritten?: (path: string, content: string) => Promise<void> | void
...
export function registerAppIpcHandlers(options: RegisterAppIpcHandlersOptions): void {
  const {
    ...
    pollWeixinInstall,
    resolveQWicksConfigPath,
    onQWicksMcpConfigWritten,
    showTurnCompleteNotification,
    getAppVersion,
    readGuiUpdateState,
  ...
  ipcMain.handle('ui-plugin:list', async () => {
    const qwicksHomeDir = join(homedir(), '.qwicks')
    await ensureBundledUiPlugins(qwicksHomeDir)
    return { plugins: await listUiPlugins(qwicksHomeDir) }
  })
  ...
  ipcMain.handle('ui-plugin:install', async () => {
    ...
    const result = await installUiPluginFromDirectory(join(homedir(), '.qwicks'), sourceDir)
    if (!result.ok) {
  ...
  ipcMain.handle('ui-plugin:remove', async (_, payload: unknown) => {
    ...
    return { ok: await removeUiPlugin(join(homedir(), '.qwicks'), request.id) }
  })
  ...
  ipcMain.handle('ui-plugin:load', async (_, payload: unknown) => {
    ...
    const qwicksHomeDir = join(homedir(), '.qwicks')
    await ensureBundledUiPlugins(qwicksHomeDir)
    return loadUiPluginFigures(qwicksHomeDir, request.id)
  })

  ipcMain.handle('qwicks:config:read', async () => {
    const path = resolveQWicksConfigPath()
    try {
  ...
  ipcMain.handle('qwicks:config:write', async (_, content: unknown) => {
    ...
    try {
      await onQWicksMcpConfigWritten?.(path, validatedContent)
    } catch (error: unknown) {
  ...
  ipcMain.handle('qwicks:config:open-dir', async () => {
    try {
      const path = resolveQWicksConfigPath()
  ...
  const resolveQWicksThreadsDataDir = async (): Promise<string> => {
    const settings = await store.load()
    const runtime = resolveQWicksRuntimeSettings(settings)
    return expandHomePath(runtime.dataDir?.trim() || DEFAULT_QWICKS_DATA_DIR)
  }

  ipcMain.handle('qwicks:sessions:detect-legacy', async () =>
    detectLegacySessions({ homeDir: homedir(), destDataDir: await resolveQWicksThreadsDataDir() })
  )

  ipcMain.handle('qwicks:sessions:import-legacy', async (_, payload: unknown) => {
    const request = parseIpcPayload('qwicks:sessions:import-legacy', legacySessionImportPayloadSchema, payload)
    try {
      const summary = await importLegacySessions({
        homeDir: homedir(),
        destDataDir: await resolveQWicksThreadsDataDir(),
  ...
  ipcMain.handle('qwicks:sessions:pick-source-dir', async (): Promise<WorkspacePickResult> => {
    const options: Electron.OpenDialogOptions = {
  ...
  ipcMain.handle('git:checkpoint:create', async (_, payload: unknown) => {
    ...
    return createGitCheckpoint({
      dataDir: await resolveQWicksThreadsDataDir(),
      workspaceRoot: request.workspaceRoot,
