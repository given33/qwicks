...
import { getRuntimeBaseUrlForSettings, runtimeAuthHeaders } from './runtime/qwicks-adapter'
...
function currentImProviderId(settings: AppSettingsV1, channel?: ClawImChannelV1): string {
  return channel?.providerId?.trim() ||
    ...
    getQWicksRuntimeSettings(settings).providerId.trim() ||
    DEFAULT_MODEL_PROVIDER_ID
}
...
function effectiveImRuntimeModel(settings: AppSettingsV1, requestedModel: string): string {
  ...
  return getQWicksRuntimeSettings(settings).model.trim() || trimmed || DEFAULT_CLAW_MODEL
}
...
export function imWelcomeText(settings: AppSettingsV1, channel?: ClawImChannelV1): string {
  ...
  const name = profile?.name.trim() || channel?.label.trim() || 'QWicks'
  const description = profile?.description.trim() ?? ''
  if (isChineseLocale(settings)) {
    return [
      `你好，我是 ${name}，通过 QWicks 连接到这个对话的 AI 助手。`,
  ...
  return [
    `Hi, I am ${name}, an AI assistant connected to this chat through QWicks.`,
    ...(description ? [description] : []),
...
export class ClawRuntime {
  ...
  private async runPrompt(settings: AppSettingsV1, options: RunPromptOptions): Promise<ClawRunResult> {
    ...
    const requestedModel = normalizeTaskModel(options.model) ?? (settings.agents.qwicks.model.trim() || DEFAULT_CLAW_MODEL)
  ...
  private async recentGeneratedFilesForThread(
    ...
    try {
      ...
      if (!detailRes.ok) {
        this.deps.logError('claw-feishu', 'Failed to read recent generated files from QWicks thread', {
          ...context,
      ...
      this.deps.logError('claw-feishu', 'Failed to inspect QWicks thread for recent generated files', {
        ...context,
        threadId: targetThreadId,
        message: errorMessage(error)
      })
  ...
  async handleTelegramUpdate(payload: TelegramInboundPayload): Promise<void> {
    ...
    // QWicks runtime attachment store is a follow-up. The downloaded file path is
  ...
  private async handleWebhook(req: IncomingMessage, res: ServerResponse): Promise<void> {
    try {
      ...
      if (url.pathname === '/claw/internal/gui-plan/create' && req.method === 'POST') {
        ...
        // native QWicks `create_plan` tool. Reject legacy calls
        // loudly so older clients see a clear migration error.
        writeJson(res, 410, {
          ok: false,
          code: 'gui_plan_create_retired',
          message:
            'The /claw/internal/gui-plan/create endpoint is no longer active. Use the QWicks create_plan tool.'
        })
        return
      }
      ...
      if (im.secret) {
        ...
        // 新名字 x-qwicks-secret 优先;旧名字 x-deepseek-gui-secret 已配置
        // 在外部系统里,属于对外契约,必须长期兼容。
        const rawHeaderSecret = req.headers['x-qwicks-secret'] ?? req.headers['x-deepseek-gui-secret']
