...
import { fetchUpstreamModelIds, readConfiguredQWicksModelIds } from './upstream-models'
...
describe('upstream model picker list', () => {
  it('includes QWicks config model profiles, aliases, and the configured agent model', async () => {
    ...
    const ids = await readConfiguredQWicksModelIds(settings(dataDir))
