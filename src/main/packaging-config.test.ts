...
describe('electron-builder QWicks packaging', () => {
  it('includes QWicks runtime dependencies in the packaged app', () => {
  ...
  it('validates the unpacked QWicks runtime before release artifacts are created', () => {
    ...
    for (const relativePath of afterPack.QWICKS_RUNTIME_REQUIRED_PATHS) {
      touch(join(unpackedRoot, relativePath))
    }
    touch(join(unpackedRoot, 'node_modules/better-sqlite3/package.json'))

    expect(() => afterPack._internals.validateBundledQWicksRuntime(context)).not.toThrow()

    rmSync(join(unpackedRoot, 'qwicks/node_modules/zod'), { recursive: true, force: true })
  ...
  it('uses the rounded QWicks icon for Windows installers and shortcuts', () => {
    // Windows ships a multi-size .ico (16/24/32/48/64/72/96/128/256) generated
    // from the rounded qwicks_mac.png so Explorer/desktop render crisp small icons
  ...
  it('checks timestamp candidates across nested macOS signed code', () => {
    const root = tempRoot()
    const appBundle = join(root, 'QWicks.app')
    const mainExecutable = join(appBundle, 'Contents/MacOS/QWicks')
