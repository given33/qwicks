...
describe('workspace-service boundary checks', () => {
  ...
  beforeEach(async () => {
    ...
    await rm(join(tmpdir(), 'qwicks'), { recursive: true, force: true })
  })
  ...
  it('reads clipboard images as PNG base64 and saves a temp file', async () => {
    ...
    expect(dirname(result.localFilePath)).toBe(join(tmpdir(), 'qwicks'))
  ...
  it('saves SDD pasted clipboard images into .qwickssdd/img with draft-relative markdown', async () => {
    const currentFilePath = join(workspaceRoot, '.qwickssdd', 'draft', 'draft-1', 'requirement.md')
    await mkdir(join(workspaceRoot, '.qwickssdd', 'draft', 'draft-1'), { recursive: true })
    ...
    const result = await saveWorkspaceClipboardImage({
      workspaceRoot,
      currentFilePath,
      imageDirectory: '.qwickssdd/img'
    })

    expect(result.ok).toBe(true)
    if (!result.ok) return

    expect(await realpath(dirname(result.path))).toBe(await realpath(join(workspaceRoot, '.qwickssdd', 'img')))
    expect(result.markdownPath.startsWith('../../img/pasted-image-')).toBe(true)
