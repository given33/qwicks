...
/**
 * Enabled, on-disk skill roots passed to the QWicks runtime. Builds the common
...
export async function guiSkillRootsForRuntime(
  ...
  const candidates = buildSkillRootCandidates(settings, workspaceRootOverride).filter((candidate) => {
    ...
    // them deliberately and QWicks tolerates missing roots); common conventions
 ...
 * added by hand to the QWicks config file.
 */
