...
import {
  ...
  ImageGenHttpError,
  type ImageGenClient
} from '../../../qwicks/src/adapters/tool/image-gen-tool-provider.js'
import { detectImage } from '../../../qwicks/src/attachments/attachment-store.js'
...
export function isWriteInfographicConfigured(
  imageGeneration: Pick<QWicksImageGenerationSettingsV1, 'enabled' | 'baseUrl' | 'apiKey' | 'model'>
): boolean {
  return (
...
function imagePromptMaxChars(imageGeneration: QWicksImageGenerationSettingsV1): number | undefined {
...
export async function requestWriteInfographic(
  ...
  const imageGeneration = resolveQWicksImageGenerationSettings(settings)
  ...
  try {
    ...
    // requirement '.qwickssdd/requirements/<id>/img').
