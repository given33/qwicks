 ...
 * so the user only has to flip the toggle rather than add QWicks by hand.
 */
type MacPermissions = {
...
/**
 * Proactively enroll QWicks in the relevant macOS permission list and open
  ...
  try {
    if (kind === 'accessibility') {
      if (native?.askForAccessibilityAccess) {
        // Registers QWicks in the Accessibility list + opens the pane.
      ...
      if (native?.askForScreenCaptureAccess) {
        // CGRequestScreenCaptureAccess: registers QWicks in the Screen
