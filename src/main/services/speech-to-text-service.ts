...
import { describeNetworkError } from '../../../qwicks/src/adapters/tool/image-gen-tool-provider.js'
...
export function isSpeechToTextConfigured(
  speechToText: Pick<QWicksSpeechToTextSettingsV1, 'enabled' | 'baseUrl' | 'apiKey' | 'model'>
): boolean {
  return (
...
export async function requestSpeechTranscription(
  ...
  const speechToText = request.speechToText ?? resolveQWicksSpeechToTextSettings(settings)
