...
describe('tray session menu', () => {
  ...
  it('groups running and recent threads with project labels', () => {
    ...
    const menu = buildTrayMenuTemplate({
      locale: 'en',
      threads: [
        thread('run', 'running', '2026-06-02T00:00:00.000Z', 'Fix tests', 'C:\\work\\QWicks'),
    ...
    expect(menu.map((item) => item.label).filter(Boolean)).toEqual([
      'Running', 'Fix tests', 'Recent', 'Review PR', 'New Chat', 'Show Desktop Pet', 'Pet Console', 'Open QWicks', 'Exit'
    ])
