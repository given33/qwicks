...
export async function fetchUpstreamModelIds(
  ...
  const configuredModelIds = await readConfiguredQWicksModelIds(settings)
...
export async function readConfiguredQWicksModelIds(settings: AppSettingsV1): Promise<string[]> {
  const runtime = resolveQWicksRuntimeSettings(settings)
