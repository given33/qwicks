...
export function resolveQWicksConfigPath(): string {
  return join(homedir(), '.qwicks', 'config.toml')
}

export function resolveDeepseekConfigPath(): string {
  return resolveQWicksConfigPath()
}

export function resolveQWicksMcpJsonPath(): string {
  return join(homedir(), '.qwicks', 'mcp.json')
}
...
async function readJsonFile(path: string): Promise<unknown | null> {
  ...
  try {
    ...
    throw new Error(`Failed to parse QWicks MCP config at ${path}: ${message}`, { cause: error })
  }
}
...
export async function syncClawScheduleMcpConfig(
  ...
  const configTomlPath = paths.configTomlPath ?? resolveQWicksConfigPath()
  const mcpJsonPath = paths.mcpJsonPath ?? resolveQWicksMcpJsonPath()
