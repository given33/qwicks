...
function installDsGui(overrides: Partial<Window['qwicksGui']>): void {
...
describe('QWicksRuntimeProvider', () => {
  it('reports the qwicks id and QWicks display name', () => {
    const provider = new QWicksRuntimeProvider()
    expect(provider.id).toBe('qwicks')
    expect(provider.displayName).toBe('QWicks')
  })
  ...
  it('maps QWicks thread items into chat blocks', async () => {
    installDsGui({
      runtimeRequest: vi.fn(async () => ({
        ok: true,
        status: 200,
  ...
  it('posts QWicks turn requests and returns the deterministic user item id', async () => {
  ...
  it('posts workspace checkpoint ids with QWicks turn requests when provided', async () => {
    const runtimeRequest = vi.fn(async () => ({
      ok: true,
      status: 202,
      body: JSON.stringify({ threadId: 'thr_1', turnId: 'turn_abc', userMessageItemId: 'item_user_real' })
  ...
  it('posts attachment ids with QWicks turn requests when provided', async () => {
    const runtimeRequest = vi.fn(async () => ({
      ok: true,
      status: 202,
  ...
  it('posts file references with QWicks turn requests when provided', async () => {
    const runtimeRequest = vi.fn(async () => ({
      ok: true,
      status: 202,
  ...
  it('posts explicit reasoning effort with QWicks turn requests', async () => {
    const runtimeRequest = vi.fn(async () => ({
      ok: true,
      status: 202,
  ...
  it('posts GUI plan context with QWicks plan turn requests', async () => {
    const runtimeRequest = vi.fn(async () => ({
      ok: true,
      status: 202,
    ...
    await provider.sendUserMessage('thr_1', 'refine the plan', {
      ...
      guiPlan: {
        ...
        planId: '/workspace/deepseek-gui:.qwickssdd/plan/auth.md',
  ...
  it('loads runtime diagnostics and uploads image attachments through QWicks endpoints', async () => {
  ...
  it('lists, disables, and deletes memory records through QWicks endpoints', async () => {
  ...
  it('calls QWicks fork and user-input compatibility endpoints', async () => {
    const runtimeRequest = vi.fn(async (path: string) => ({
      ok: true,
      status: 200,
  ...
  it('resumes a session through the QWicks HTTP runtime', async () => {
    const runtimeRequest = vi.fn(async () => ({
      ok: true,
      status: 201,
    ...
    expect(runtimeRequest).toHaveBeenCalledWith(
      ...
      JSON.stringify({
        workspace: '/tmp/workspace',
        model: defaultQWicksRuntimeSettings().model,
        mode: 'plan'
      })
    )
  })

  it('maps QWicks SSE deltas into the thread event sink', async () => {
  ...
  it('auto-approves approval requests when policy is auto', async () => {
    ...
    const autoSettings: AppSettingsV1 = {
      ...settings(),
      agents: { qwicks: { ...defaultQWicksRuntimeSettings(), approvalPolicy: 'auto' } }
    }
    installDsGui({
  ...
  it('uses the approval policy from runtime events before falling back to settings', async () => {
    ...
    const getSettings = vi.fn(async (): Promise<AppSettingsV1> => ({
      ...settings(),
      agents: { qwicks: { ...defaultQWicksRuntimeSettings(), approvalPolicy: 'on-request' } }
    }))
    const ac = new AbortController()
    const sink: ThreadEventSink = {
      onSeq: vi.fn(),
  ...
  it('renders approval cards for suggest and untrusted policies', async () => {
    for (const policy of ['suggest', 'untrusted'] as const) {
      ...
      const policySettings: AppSettingsV1 = {
        ...settings(),
        agents: { qwicks: { ...defaultQWicksRuntimeSettings(), approvalPolicy: policy } }
      }
      installDsGui({
...
describe('registry', () => {
  it('returns a cached provider for the qwicks id', () => {
