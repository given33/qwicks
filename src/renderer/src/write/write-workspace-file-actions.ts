...
export function createWriteFileActions({
  ...
  return {
    ...
    loadDirectory: async (workspaceRoot, path) => {
      ...
      let result: Awaited<ReturnType<typeof window.qwicksGui.listWorkspaceDirectory>>
      try {
        result = await window.qwicksGui.listWorkspaceDirectory({ workspaceRoot, path })
      } catch (error) {
        set((state) => ({
    ...
    openFile: async (workspaceRoot, path) => {
      ...
      try {
        if (isWriteImageFilePath(path)) {
          const result = await window.qwicksGui.readWorkspaceImage({ path, workspaceRoot })
          if (!result.ok) {
        ...
        if (isWritePdfFilePath(path)) {
          const result = await window.qwicksGui.readWorkspacePdf({ path, workspaceRoot })
          if (!result.ok) {
            set({ fileLoading: false, fileError: result.message })
            return
          }
        ...
        const result = await window.qwicksGui.readWorkspaceFile({ path, workspaceRoot })
        if (!result.ok) {
          set({ fileLoading: false, fileError: result.message })
          return
        }
    ...
    createFile: async (workspaceRoot, path, content = '') => {
      let result: Awaited<ReturnType<typeof window.qwicksGui.createWorkspaceFile>>
      try {
        result = await window.qwicksGui.createWorkspaceFile({ workspaceRoot, path, content })
      } catch (error) {
    ...
    createDirectory: async (workspaceRoot, path) => {
      let result: Awaited<ReturnType<typeof window.qwicksGui.createWorkspaceDirectory>>
      try {
        result = await window.qwicksGui.createWorkspaceDirectory({ workspaceRoot, path })
      } catch (error) {
        set({ fileError: formatActionError(error) })
        return null
      }
    ...
    renameEntry: async (workspaceRoot, path, newName) => {
      cancelExternalSyncAnimation()
      let result: Awaited<ReturnType<typeof window.qwicksGui.renameWorkspaceEntry>>
      try {
        result = await window.qwicksGui.renameWorkspaceEntry({ workspaceRoot, path, newName })
      } catch (error) {
        set({ fileError: formatActionError(error) })
        return null
      }
    ...
    deleteEntry: async (workspaceRoot, path) => {
      cancelExternalSyncAnimation()
      let result: Awaited<ReturnType<typeof window.qwicksGui.deleteWorkspaceEntry>>
      try {
        result = await window.qwicksGui.deleteWorkspaceEntry({ workspaceRoot, path })
      } catch (error) {
        set({ fileError: formatActionError(error) })
        return false
      }
