...
function image(partial: Partial<SddDraftImageReference> = {}): SddDraftImageReference {
  return {
    ...
    relativePath: '.qwickssdd/img/wireframe.png',
    mimeType: 'image/png',
...
describe('buildSddDraftToPlanPrompt', () => {
  it('keeps Markdown image syntax and maps visual attachments by image number', () => {
    const prompt = buildSddDraftToPlanPrompt({
      ...
      draftRelativePath: '.qwickssdd/draft/draft-1/requirement.md',
      planRelativePath: '.qwickssdd/plan/sdd-draft-1.md',
    ...
    expect(prompt).toContain('Reserved plan file: .qwickssdd/plan/sdd-draft-1.md')
    expect(prompt).toContain('`plan_relative_path` to `.qwickssdd/plan/sdd-draft-1.md`')
  })
