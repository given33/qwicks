...
function directory(entries: Array<{ name: string, type: 'file' | 'directory', path?: string }>): WorkspaceDirectoryListResult {
  return {
    ok: true,
    root: '/tmp/app/.qwickssdd/requirements',
    entries: entries.map((entry) => ({
      name: entry.name,
      path: entry.path ?? `/tmp/app/.qwickssdd/requirements/${entry.name}`,
      type: entry.type,
...
describe('sdd-draft-history', () => {
  ...
  it('discovers disk-only requirement drafts under .qwickssdd/requirements', async () => {
    ...
    const listWorkspaceDirectory = vi.fn(async (options: WorkspaceDirectoryTarget) => {
      if (options.path === '.qwickssdd/requirements') {
        return directory([
      ...
      if (options.path === `.qwickssdd/requirements/${draftId}`) {
        return directory([
          {
            name: 'requirement.md',
            type: 'file',
            path: `/tmp/app/.qwickssdd/requirements/${draftId}/requirement.md`
          }
        ])
      }
    ...
    expect(history[0]).toMatchObject({
      ...
      relativePath: `.qwickssdd/requirements/${draftId}/requirement.md`,
