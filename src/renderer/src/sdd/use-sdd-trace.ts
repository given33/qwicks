...
export function useSddTrace(input: {
  ...
  useEffect(() => {
    ...
    const load = async (): Promise<void> => {
      if (typeof window.qwicksGui?.readWorkspaceFile !== 'function') return
      const requirement = await window.qwicksGui
      ...
      if (!planIsActive) {
        const plan = await window.qwicksGui
      ...
      if (tracePath) {
        const trace = await window.qwicksGui
  ...
  useEffect(() => {
    ...
    const run = async (): Promise<void> => {
      try {
        ...
        if (typeof window.qwicksGui?.writeWorkspaceFile !== 'function') return
