...
async function writeSddChatMeta(input: {
  ...
  try {
    const existing = await window.qwicksGui.readWorkspaceFile({
  ...
  await window.qwicksGui.writeWorkspaceFile({
    workspaceRoot: input.workspaceRoot,
    path: metaPath,
...
export async function writeSddChatTranscriptForThread(input: {
  ...
  if (typeof window.qwicksGui?.writeWorkspaceFile !== 'function') return false
