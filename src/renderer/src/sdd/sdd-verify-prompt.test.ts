...
describe('buildSddVerifyPrompt', () => {
  const prompt = buildSddVerifyPrompt({
    ...
    draftRelativePath: '.qwickssdd/requirements/abc/requirement.md',
    planRelativePath: '.qwickssdd/plan/sdd-abc.md'
  })

  it('keeps the intro and core in-place verification instructions', () => {
    expect(prompt).toContain(SDD_VERIFY_INTRO)
    expect(prompt).toContain('Requirement file: .qwickssdd/requirements/abc/requirement.md')
