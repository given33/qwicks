...
function currentPlatform(): string {
  return typeof window !== 'undefined' ? window.qwicksGui?.platform ?? '' : ''
}

function currentHomeDir(): string {
  return typeof window !== 'undefined' ? window.qwicksGui?.homeDir ?? '' : ''
}
...
export function expandSettingsHomePathsForUse(
  ...
  return {
    ...
    agents: {
      ...
      qwicks: {
        ...qwicks,
        binaryPath: expand(qwicks.binaryPath),
        dataDir: expand(qwicks.dataDir),
        storage: {
          ...qwicks.storage,
          sqlitePath: expand(qwicks.storage.sqlitePath)
        }
      }
    },
    write: {
