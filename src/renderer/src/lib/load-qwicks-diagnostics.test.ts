...
import { loadQWicksDiagnostics } from './load-qwicks-diagnostics'

describe('loadQWicksDiagnostics', () => {
  it('loads runtime info, tool diagnostics, and memory records together', async () => {
    ...
    const loaded = await loadQWicksDiagnostics(provider, { workspace: '/tmp/project' })
