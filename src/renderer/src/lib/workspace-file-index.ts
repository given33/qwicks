...
async function buildWorkspaceFileIndex(root: string): Promise<WorkspaceFileIndex> {
  ...
  while (
    ...
    const result = await window.qwicksGui.listWorkspaceDirectory({ workspaceRoot: root, path: current.path })
...
export async function loadWorkspaceMentionPathSuggestions(
  ...
  const task = (async () => {
    const result = await window.qwicksGui.listWorkspaceDirectory({ workspaceRoot: root, path: dir })
    if (!result.ok) return []
