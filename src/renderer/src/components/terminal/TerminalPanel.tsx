...
export function TerminalPanel({ className = '', workspaceRoot, onCollapse, height }: Props): ReactElement {
  ...
  const attachTerminal = useCallback(async (tabId: string) => {
    ...
    const offData = window.qwicksGui.onTerminalData((payload) => {
      if (payload.sessionId !== sessionId) return
      term.write(payload.data)
    })
    const offExit = window.qwicksGui.onTerminalExit((payload) => {
      if (payload.sessionId !== sessionId) return
      setExited(true)
    })
    ...
    const disposable = term.onData((data) => {
      void window.qwicksGui.writeToTerminal({
        sessionId,
        data
      })
    })
    ...
    const onDimensionChange = (dim: { cols: number; rows: number }): void => {
      void window.qwicksGui.resizeTerminal({
        sessionId,
        cols: dim.cols,
        rows: dim.rows
      })
    ...
    try {
      const result = await window.qwicksGui.createTerminal({
        sessionId,
  ...
  const handleCloseTab = useCallback((tabId: string) => {
    ...
    void window.qwicksGui.disposeTerminal(sessionIdForTab(tabId))
    setTabs((current) => {
  ...
  const handleCloseOtherTabs = useCallback((tabId: string) => {
    ...
    for (const tab of tabs) {
      if (tab.id !== tabId) void window.qwicksGui.disposeTerminal(sessionIdForTab(tab.id))
    }
    setTabs([keptTab])
    setActiveTabId(tabId)
    setContextMenu(null)
  ...
  const handleCloseAllTabs = useCallback(() => {
    for (const tab of tabs) {
      void window.qwicksGui.disposeTerminal(sessionIdForTab(tab.id))
    }
    setContextMenu(null)
    cancelRenameTab()
  ...
  const handleRestart = useCallback(async () => {
    ...
    try {
      await window.qwicksGui.disposeTerminal(sessionIdForTab(activeTab.id))
    } catch {
      /* ignore */
    }
    setError(null)
