...
export function WorkflowView({ leftSidebarCollapsed, onToggleLeftSidebar }: Props): ReactElement {
  ...
  const refreshStatus = useCallback(async (): Promise<void> => {
    if (typeof window.qwicksGui?.getWorkflowStatus !== 'function') return
    try {
      setStatus(await window.qwicksGui.getWorkflowStatus())
    } catch {
  ...
  const load = useCallback(async (): Promise<void> => {
    try {
      const [nextSettings, nextStatus] = await Promise.all([
        ...
        typeof window.qwicksGui?.getWorkflowStatus === 'function'
          ? window.qwicksGui.getWorkflowStatus()
  ...
  const handleExport = useCallback((workflow: WorkflowV1): void => {
    const text = serializeWorkflowDsl(workflow, 'qwicks', new Date().toISOString())
  ...
  const handleRun = useCallback(
    async (id: string, input?: Record<string, unknown>): Promise<void> => {
      if (typeof window.qwicksGui?.runWorkflow !== 'function') return
      const result = await window.qwicksGui.runWorkflow(id, input)
      if (!result.ok) {
  ...
  const handleStop = useCallback(
    async (id: string): Promise<void> => {
      if (typeof window.qwicksGui?.stopWorkflow !== 'function') return
      await window.qwicksGui.stopWorkflow(id)
      void refreshStatus()
    },
    [refreshStatus]
  )
  ...
  const handleRunNode = useCallback(
    async (workflowId: string, nodeId: string): Promise<void> => {
      if (typeof window.qwicksGui?.runWorkflowNode !== 'function') return
      const result = await window.qwicksGui.runWorkflowNode(workflowId, nodeId)
      if (!result.ok) {
        setError(result.message)
        return
      }
