...
export function WriteWorkspaceView({
  ...
  const submitInlineEdit = async (prompt: string): Promise<void> => {
    ...
    if (typeof window.qwicksGui?.requestWriteInlineCompletion !== 'function') {
    ...
    try {
      const result = await window.qwicksGui.requestWriteInlineCompletion(
  ...
  const generateInfographic = (): void => {
    ...
    if (typeof window.qwicksGui?.generateWriteInfographic !== 'function') {
  ...
  const completeInfographicGeneration = async (job: {
    ...
    try {
      const result = await window.qwicksGui.generateWriteInfographic({
        text: job.text,
  ...
  const resolveInfographicPlaceholder = async (
    ...
    if (
      typeof window.qwicksGui?.readWorkspaceFile !== 'function' ||
      typeof window.qwicksGui?.writeWorkspaceFile !== 'function'
    ) {
      return false
    }
    try {
      const file = await window.qwicksGui.readWorkspaceFile({ path: job.filePath, workspaceRoot })
      ...
      if (next === null) return false
      const written = await window.qwicksGui.writeWorkspaceFile({
        path: job.filePath,
        workspaceRoot,
        content: next
      })
  ...
  const pickWriteWorkspace = async (): Promise<void> => {
    try {
      ...
      const picked = await window.qwicksGui.pickWorkspaceDirectory(workspaceRoot || undefined)
  ...
  const exportCurrentFile = async (format: WriteExportFormat): Promise<void> => {
    ...
    if (typeof window.qwicksGui?.exportWriteDocument !== 'function') {
    ...
    try {
      const result = await window.qwicksGui.exportWriteDocument({
        path: activeFilePath,
        workspaceRoot,
        format,
        content: fileContent
  ...
  const copyCurrentFileAsRichText = async (): Promise<void> => {
    if (!activeFilePath) return
    if (!activeFileIsText) return
    if (typeof window.qwicksGui?.copyWriteDocumentAsRichText !== 'function') {
    ...
    try {
      const result = await window.qwicksGui.copyWriteDocumentAsRichText({
        path: activeFilePath,
        workspaceRoot,
        content: fileContent
      })
  ...
  useEffect(() => {
    ...
    if (
      typeof window.qwicksGui?.watchWorkspaceFile !== 'function' ||
      typeof window.qwicksGui?.unwatchWorkspaceFile !== 'function' ||
      typeof window.qwicksGui?.onWorkspaceFileChanged !== 'function'
    ) {
      return
    }

    return startWriteWorkspaceFileWatch({
      api: window.qwicksGui,
      workspaceRoot,
      path: activeFilePath,
