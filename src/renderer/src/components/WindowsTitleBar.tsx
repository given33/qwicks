...
function currentPlatform(): string {
  return typeof window !== 'undefined' ? window.qwicksGui?.platform ?? 'unknown' : 'unknown'
}

function defaultRunDesktopCommand(command: DesktopCommand): Promise<void> {
  if (typeof window === 'undefined' || typeof window.qwicksGui?.runDesktopCommand !== 'function') {
    return Promise.resolve()
  }
  return window.qwicksGui.runDesktopCommand(command)
}

function defaultOpenLogDir(): Promise<void> {
  if (typeof window === 'undefined' || typeof window.qwicksGui?.openLogDir !== 'function') {
    return Promise.resolve()
  }
  return window.qwicksGui.openLogDir().then(() => undefined)
}
...
export function WindowsTitleBar({ platform, actions }: Props): ReactElement | null {
  ...
  const defaultActions = useMemo<WindowsTitleBarActions>(() => ({
    ...
    showAbout: async () => {
      const version =
        typeof window !== 'undefined' && typeof window.qwicksGui?.getAppVersion === 'function'
          ? await window.qwicksGui.getAppVersion().catch(() => '')
          : ''
  ...
  return (
    <div ref={rootRef} data-tauri-drag-region data-cursor-spotlight-target className="ds-windows-titlebar ds-drag">
      <div className="ds-windows-titlebar-content">
        <img src={qwicksLogo} alt="" aria-hidden="true" className="ds-windows-titlebar-icon" />
