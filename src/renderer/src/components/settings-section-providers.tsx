...
import type { ModelProviderProbeResult } from '@shared/qwicks-gui-api'
import {
  AudioLines,
  ChevronDown,
  Clapperboard,
...
export function modelProvidersSettingsPatch(input: {
  ...
  const miniMaxMediaDefaults = defaultMiniMaxMediaGenerationQWicksPatch({
    ...
    currentQWicks: input.currentQWicks,
    qwicksPatch: input.qwicks
  })
  const baseQWicksPatch = input.qwicks?.providerId?.trim()
    ? { ...input.qwicks, apiKey: '', baseUrl: '' }
    : input.qwicks ?? {}
  const qwicksPatch = {
    ...baseQWicksPatch,
  ...
  return {
    ...
    ...(Object.keys(qwicksPatch).length > 0 ? { agents: { qwicks: qwicksPatch } } : {})
  }
}
...
export function ProvidersSettingsSection({ ctx }: { ctx: Record<string, any> }): ReactElement {
  ...
  const [selectedProviderId, setSelectedProviderId] = useState<string>(
    qwicks.providerId?.trim() || modelProviders[0]?.id || DEFAULT_MODEL_PROVIDER_ID
  )
  ...
  const activeQWicksProviderId: string = qwicks.providerId?.trim() || DEFAULT_MODEL_PROVIDER_ID
  ...
  const confirmAction = async (options: {
    ...
    if (typeof window.qwicksGui?.confirmDialog === 'function') {
      return window.qwicksGui.confirmDialog(options)
    }
    return true
  }
  ...
  const updateModelProviderId = (id: string, value: string): void => {
    ...
    updateModelProviders(
      ...
      qwicks.providerId === id ? { providerId: nextId } : undefined
    )
  }
  ...
  const commitProviderDraft = (): void => {
    ...
    updateModelProviders(
      ...
      hasKey
        ? { providerId: draftProvider.id, model: draftProvider.models[0] ?? qwicks.model }
        : undefined
    )
  ...
  const cancelProviderDraft = (): void => {
    if (!draftProvider) return
    setDraftProvider(null)
    setSelectedProviderId(activeQWicksProviderId)
  }
  ...
  const addPresetModelProvider = async (
    ...
    updateModelProviders(
      ...
      nextProvider.apiKey.trim()
        ? { providerId: nextProvider.id, model: nextProvider.models[0] ?? qwicks.model }
        : undefined
    )
  }
  ...
  const runProbe = async (target: ModelProviderProfileV1, mode: 'test' | 'fetch'): Promise<void> => {
    if (typeof window.qwicksGui?.probeModelProvider !== 'function') return
    ...
    try {
      result = await window.qwicksGui.probeModelProvider({
  ...
  const renderProviderButton = (item: ModelProviderProfileV1): ReactElement => {
    ...
    const inUse = !isDraft && activeQWicksProviderId === item.id
