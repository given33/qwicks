...
function settingsWithActiveXiaomiWithoutKey(): AppSettingsV1 {
  return settings({
    ...
    agents: { qwicks: { providerId: 'xiaomi' } }
  })
}
...
describe('initialSetupSelection', () => {
  ...
  it('preselects the token plan mode for token plan profiles', () => {
    const current = settings({ agents: { qwicks: { providerId: 'minimax-token-plan' } } })
  ...
  it('falls back to deepseek for unknown or empty active providers', () => {
    ...
    expect(initialSetupSelection(settings({ agents: { qwicks: { providerId: 'custom-provider-2' } } })))
      .toEqual({ presetId: 'deepseek', mode: 'api' })
    expect(initialSetupSelection(settings({ agents: { qwicks: { providerId: 'litellm' } } })))
      .toEqual({ presetId: 'deepseek', mode: 'api' })
  })
})
...
describe('initialSetupDrafts', () => {
  ...
  it('keeps coding and Moonshot presets out of onboarding', () => {
    ...
    for (const id of excludedIds) {
      ...
      expect(initialSetupSelection(settings({ agents: { qwicks: { providerId: id } } })))
        .toEqual({ presetId: 'deepseek', mode: 'api' })
    }
  })
})
...
describe('buildInitialSetupSettings', () => {
  it('activates deepseek so the boot gate sees the key the user typed', () => {
    ...
    expect(getQWicksRuntimeSettings(next).providerId).toBe('deepseek')
  ...
  it('syncs the deepseek draft into the provider profile used by settings', () => {
    const current = settings({
      ...
      agents: { qwicks: { providerId: 'deepseek' } }
    })
    const drafts = initialSetupDrafts(current)
    drafts.deepseek = {
      apiKey: 'sk-new',
  ...
  it('keeps the model override when the provider does not change', () => {
    const current = settings({
      ...
      agents: { qwicks: { providerId: 'deepseek', model: 'deepseek-v4-flash' } }
    })
    const next = buildInitialSetupSettings(current, initialSetupDrafts(current), {
      presetId: 'deepseek',
      mode: 'api'
    })
    expect(getQWicksRuntimeSettings(next).model).toBe('deepseek-v4-flash')
  })
