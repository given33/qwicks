...
import type { DesktopCommand, ModelProviderModelGroup, SkillListItem } from '@shared/qwicks-gui-api'
...
import type { CoreRuntimeInfoJson, CoreRuntimeSkillJson } from '../agent/qwicks-contract'
...
import { IqwicksCameoLayer, QWicksCelebrationLayer } from './chat/AnimatedWorkLogo'
import {
  FloatingComposer,
...
export function Workbench(): ReactElement {
  ...
  const shortcutPlatform = typeof window === 'undefined' ? undefined : window.qwicksGui?.platform
  ...
  useEffect(() => {
    const runDesktopShortcut = (command: DesktopCommand): void => {
      if (typeof window.qwicksGui?.runDesktopCommand !== 'function') return
      void window.qwicksGui.runDesktopCommand(command)
    }
  ...
  useEffect(() => {
    if (typeof window === 'undefined' || typeof window.qwicksGui?.getLogPath !== 'function') return
    let cancelled = false
    ...
    void rendererRuntimeClient.getSettings()
      .then((settings) => {
        ...
        setComposerExecutionSettings({
          approvalPolicy: settings.agents.qwicks.approvalPolicy,
          sandboxMode: settings.agents.qwicks.sandboxMode
        })
      })
      .catch(() => undefined)
    return () => {
  ...
  const mirrorClawCommand = async (userText: string, replyText: string): Promise<void> => {
    if (!activeThreadId || typeof window.qwicksGui?.mirrorClawChannelMessage !== 'function') return
    const userResult = await window.qwicksGui.mirrorClawChannelMessage(
      activeThreadId,
      userText,
      'user'
    )
    if (!userResult.ok) return
    await window.qwicksGui.mirrorClawChannelMessage(
      activeThreadId,
      replyText,
      'assistant'
    )
  ...
  useEffect(() => {
    ...
    const localSkillsTask = typeof window !== 'undefined' && typeof window.qwicksGui?.listSkills === 'function'
      ? window.qwicksGui.listSkills(activeSkillWorkspace || undefined)
    ...
    try {
      ...
      for (const [index, file] of files.entries()) {
        ...
        const localFilePath =
          ...
          (typeof window.qwicksGui?.getPathForFile === 'function' ? window.qwicksGui.getPathForFile(file) : '')
  ...
  const handlePasteClipboardImage = async (options: { silentNoImage?: boolean } = {}): Promise<void> => {
    ...
    if (typeof window.qwicksGui?.readClipboardImage !== 'function') {
      setAttachmentUploadError(t('composerAttachmentUnavailable'))
      return
    }
    const image = await window.qwicksGui.readClipboardImage()
    if (!image.ok) {
  ...
  const sendWritePrompt = (value: string): void => {
    ...
    void (async () => {
      ...
      if (retrievalQuery && typeof window.qwicksGui?.retrieveWriteContext === 'function') {
        try {
          const result = await window.qwicksGui.retrieveWriteContext({
          ...
          void window.qwicksGui?.logError?.('write-retrieval', 'Failed to retrieve write context', {
  ...
  const startNewSddRequirement = async (): Promise<void> => {
    ...
    const restored = await restoreRememberedSddDraft({
      ...
      readWorkspaceFile: window.qwicksGui.readWorkspaceFile
    })
    ...
    const result = await window.qwicksGui.createWorkspaceFile({
      workspaceRoot: targetWorkspace,
  ...
  const findSddDraftForSidebarThread = async (
    ...
    const listWorkspaceDirectory = window.qwicksGui?.listWorkspaceDirectory
    const readWorkspaceFile = window.qwicksGui?.readWorkspaceFile
  ...
  const sendSddPrototypeTurn = async (payload: {
    ...
    if (payload.image) {
      try {
        const read = await window.qwicksGui.readWorkspaceImage({
  ...
  const readComposerFileContextEntries = async (
    ...
    const appendFileEntry = async (
      ...
      const result = await window.qwicksGui.readWorkspaceFile({
  ...
  const handleSendAsync = async (): Promise<void> => {
    ...
    if (route === 'claw') {
      ...
      setInput('')
      void (async () => {
        const taskResult = typeof window.qwicksGui?.createClawTaskFromText === 'function'
          ? await window.qwicksGui.createClawTaskFromText(v, {
  ...
  const pickWriteAssistantWorkspace = async (): Promise<void> => {
    try {
      ...
      if (typeof window.qwicksGui?.pickWorkspaceDirectory !== 'function') {
        throw new Error('workspace:pick-directory unavailable')
      }
      const picked = await window.qwicksGui.pickWorkspaceDirectory(
  ...
  const renderRuntimeBanner = (message: string, detail?: string | null): ReactElement => (
    <RuntimeBanner
      ...
      t={t}
      onOpenLogDir={
        typeof window !== 'undefined' && typeof window.qwicksGui?.openLogDir === 'function'
          ? () => window.qwicksGui.openLogDir()
          : undefined
      }
