...
const INSTALLED_STORAGE_KEY = 'qwicks.installedPlugins'
...
export function PluginMarketplaceView(): ReactElement {
  ...
  const readMcpConfig = useCallback(async (): Promise<string> => {
    if (typeof window.qwicksGui?.getQWicksConfigFile !== 'function') return mcpConfigText
    const file = await window.qwicksGui.getQWicksConfigFile()
  ...
  const refreshMcpRuntimeOverlay = useCallback(async (): Promise<void> => {
    if (typeof window.qwicksGui?.runtimeRequest !== 'function') {
      setRuntimeInfo(null)
  ...
  const refreshSkillList = useCallback(async (): Promise<void> => {
    if (typeof window.qwicksGui?.listSkills !== 'function') {
      setDiscoveredSkills([])
  ...
  const refreshSkillRoots = useCallback(async (): Promise<void> => {
    if (typeof window.qwicksGui?.listSkillRoots !== 'function') {
      setSkillRoots([])
      return
    }
    try {
  ...
  const appendMcpConfig = async (id: string, config: JsonRecord): Promise<void> => {
    ...
    const result = await window.qwicksGui.setQWicksConfigFile(merged.text)
  ...
  const addItem = async (item: MarketplaceItem): Promise<void> => {
    ...
    try {
      ...
      const result = await window.qwicksGui.saveSkillFile(selectedSkillRoot.path, item.id, content)
      if (!result.ok) {
  ...
  const addCustom = async (): Promise<void> => {
    ...
    try {
      if (activeKind === 'mcp') {
        ...
        const result = await window.qwicksGui.saveSkillFile(selectedSkillRoot.path, id, content)
        if (!result.ok) {
          setNotice({ tone: 'error', message: result.message })
          return
        }
  ...
  const toggleMcpEnabled = async (id: string, enabled: boolean): Promise<void> => {
    ...
    try {
      ...
      await window.qwicksGui.setQWicksConfigFile(nextText)
  ...
  const openManageTarget = async (): Promise<void> => {
    try {
      ...
      const result = await window.qwicksGui.openSkillRoot(selectedSkillRoot.path)
      if (!result.ok) setNotice({ tone: 'error', message: result.message ?? t('pluginActionFailed') })
    } catch (e) {
      setNotice({ tone: 'error', message: e instanceof Error ? e.message : String(e) })
    }
...
function runtimeOverlayErrorMessage(error: unknown, fallback: string): string {
  const message = error instanceof Error ? error.message : String(error)
  return /runtimeRequest|qwicksGui|Cannot read properties/i.test(message) ? fallback : message
}
