...
export function ScheduleTasksView({
  ...
  const load = useCallback(async (): Promise<void> => {
    try {
      const [nextSettings, nextStatus] = await Promise.all([
        ...
        typeof window.qwicksGui?.getScheduleStatus === 'function'
          ? window.qwicksGui.getScheduleStatus()
  ...
  const persistSchedule = async (patch: Parameters<typeof mergeScheduleSettings>[1]): Promise<void> => {
    ...
    setSettings(saved)
    if (typeof window.qwicksGui?.getScheduleStatus === 'function') {
      setStatus(await window.qwicksGui.getScheduleStatus())
    }
  }
  ...
  const runTask = async (taskId: string): Promise<void> => {
    if (typeof window.qwicksGui?.runScheduleTask !== 'function') return
    const result = await window.qwicksGui.runScheduleTask(taskId)
    if (!result.ok) {
