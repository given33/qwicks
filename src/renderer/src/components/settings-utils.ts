...
export const DEFAULT_WORKSPACE_ROOT = '~/.qwicks/default_workspace'
...
export function hasValidPort(settings: AppSettingsV1): boolean {
  const port = getQWicksRuntimeSettings(settings).port
...
export function mergeSettings(current: AppSettingsV1, patch: SettingsPatch): AppSettingsV1 {
  ...
  return {
    ...applyQWicksRuntimePatch(safeCurrent, agentsPatch?.qwicks),
    ...restPatch,
...
export function coerceRendererSettings(settings: AppSettingsV1): AppSettingsV1 {
  ...
  return {
    ...
    agents: qwicksSettingsEnvelope(mergeQWicksRuntimeSettings(defaultQWicksRuntimeSettings(), getQWicksRuntimeSettings(settings))),
