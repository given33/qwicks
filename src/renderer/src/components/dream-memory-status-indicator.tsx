 ...
 *
 * Batch C(§3.2):(N 个来源) 可点击展开 MemorySourcesPanel;共享视图整体不渲染。
 * 使用 <details> 原生折叠(DOM-native),无需 React state —— 既支持组件渲染,
 * 也兼容测试里直接函数调用(不触发 hook 规则)。
 *
...
export function DreamMemoryStatusIndicator({
  ...
  const hasSources = !!status.sources && status.sources.sourceIds.length > 0
  ...
  return (
    <div className="mt-1">
      <div className="flex items-center gap-2 text-xs text-zinc-400">
        ...
        {hasSources && (
          <span className="text-zinc-500">({status.sources!.sourceIds.length} 个来源)</span>
        )}
      </div>
      {hasSources && status.sources && (
        <MemorySourcesPanel
          sources={{
            usedMemoryIds: status.sources.usedMemoryIds ?? [],
            downrankedMemoryIds: status.sources.downrankedMemoryIds ?? [],
            suppressedMemoryIds: status.sources.suppressedMemoryIds ?? [],
            sourceIds: status.sources.sourceIds ?? []
          }}
