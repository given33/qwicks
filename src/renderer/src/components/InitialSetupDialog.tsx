...
type ModelConfigurationPanelVariant = 'dialog' | 'settings'
...
export function ModelConfigurationPanel({
  ...
  mode: InitialSetupMode
  variant?: ModelConfigurationPanelVariant
}): ReactElement {
  ...
  const activeMode: InitialSetupMode = isSettings ? 'preview' : mode
  const isPreview = activeMode === 'preview'
  const closeAllowed = variant === 'dialog' && canCloseInitialSetup(activeMode)
  ...
  const handleOpenKeyPage = (url: string) => {
    if (typeof window.qwicksGui?.openExternal !== 'function') return
    void window.qwicksGui.openExternal(url).catch(() => undefined)
  }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (completed && isSettings) setSaved(true)
    } catch (e) {
  ...
  if (!form || !drafts) {
    if (isSettings) {
      return (
        <section className="rounded-2xl border border-ds-border bg-ds-card/95 px-5 py-4 text-sm text-ds-muted shadow-sm">
          {t('loading')}
        </section>
      )
    }
  ...
  const titleId = isSettings ? 'model-configuration-title' : 'initial-setup-title'
  const panelTitle = isSettings ? t('modelConfigurationTitle') : t('firstRunTitle')
  const panelSubtitle = isSettings ? t('modelConfigurationSubtitle') : t('firstRunSubtitle')
  return (
    <div className={isSettings ? 'ds-no-drag' : 'ds-no-drag fixed inset-0 z-50 overflow-y-auto bg-[#eef2fb]/45 p-3 backdrop-blur-[18px] dark:bg-black/62 dark:backdrop-blur-[22px] sm:p-6'}>
      <div className={isSettings ? '' : 'flex min-h-full items-center justify-center'}>
        <section
          role={isSettings ? undefined : 'dialog'}
          aria-modal={isSettings ? undefined : true}
          aria-labelledby={titleId}
          className={
            isSettings
              ? 'overflow-hidden rounded-2xl border border-white/75 bg-[rgba(255,255,255,0.94)] text-slate-900 shadow-sm shadow-black/5 backdrop-blur-2xl dark:border-white/10 dark:bg-[rgba(18,21,28,0.96)] dark:text-white dark:shadow-black/25'
              : 'flex h-[calc(100dvh-24px)] max-h-[calc(100dvh-24px)] w-full max-w-[640px] flex-col overflow-hidden rounded-2xl border border-white/75 bg-[rgba(255,255,255,0.94)] text-slate-900 shadow-[0_28px_86px_rgba(88,105,136,0.22)] backdrop-blur-2xl dark:border-white/10 dark:bg-[rgba(18,21,28,0.96)] dark:text-white dark:shadow-[0_28px_92px_rgba(0,0,0,0.55)] sm:h-auto sm:max-h-[calc(100dvh-48px)]'
          }
        >
        <div className="shrink-0 border-b border-slate-200/72 bg-[linear-gradient(180deg,rgba(255,255,255,0.96),rgba(248,250,253,0.9))] px-5 py-4 dark:border-white/10 dark:bg-[linear-gradient(180deg,rgba(27,31,40,0.98),rgba(19,22,29,0.96))] sm:px-7 sm:py-6">
          <div className="flex items-start justify-between gap-3">
            <div className="inline-flex min-w-0 items-center gap-2 rounded-lg border border-[#1388ff]/22 bg-[#1388ff]/[0.06] px-3 py-1.5 text-[12.5px] font-semibold text-[#1377df] dark:border-[#3aa0ff]/22 dark:bg-[#3aa0ff]/[0.12] dark:text-[#88c8ff]">
              ...
              <span className="min-w-0 truncate">{isSettings ? t('modelConfiguration') : t(isPreview ? 'firstRunPreviewBadge' : 'firstRunBadge')}</span>
            </div>
            {closeAllowed ? (
              <button
                type="button"
          ...
          <h1 id={titleId} className="mt-3 text-xl font-semibold leading-tight text-slate-900 dark:text-white sm:mt-4 sm:text-[22px]">
            {panelTitle}
          </h1>
        ...
        <div className={isSettings ? 'space-y-4 px-5 py-4 sm:space-y-5 sm:px-7 sm:py-6' : 'min-h-0 flex-1 space-y-4 overflow-y-auto px-5 py-4 sm:space-y-5 sm:px-7 sm:py-6'}>
          {!isSettings ? (
          <>
        ...
        <div className="shrink-0 space-y-3 border-t border-slate-200/72 bg-white/70 px-5 pb-4 pt-3.5 dark:border-white/10 dark:bg-white/[0.025] sm:space-y-4 sm:px-7 sm:pb-6 sm:pt-4">
          ...
          {saved ? (
            <div className="rounded-xl border border-emerald-300/60 bg-emerald-50/80 px-4 py-3 text-[13px] text-emerald-700 dark:border-emerald-800/60 dark:bg-emerald-950/30 dark:text-emerald-300">
              {t('modelConfigurationSaved')}
            </div>
          ) : null}
          ...
          <div className={closeAllowed ? 'flex flex-col-reverse gap-3 sm:grid sm:grid-cols-[0.85fr_1fr]' : 'grid gap-3'}>
            ...
            >
              {saving ? t('firstRunSaving') : t(isSettings ? 'modelConfigurationSave' : 'firstRunSave')}
            </button>
          </div>

          <p className="text-center text-[12.5px] leading-6 text-slate-400 dark:text-slate-500">
            {t(isSettings ? 'modelConfigurationHint' : isPreview ? 'firstRunPreviewHint' : 'firstRunChangeLater')}
          </p>
        </div>
        </section>
      </div>
...
export function InitialSetupDialog(): ReactElement {
  ...
  return <ModelConfigurationPanel mode={initialSetupMode} variant="dialog" />
}
