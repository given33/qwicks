...
function StreamdownLink({
  ...
  const handleClick = (event: MouseEvent<HTMLAnchorElement>): void => {
    ...
    if (isExternal && href && typeof window.qwicksGui?.openExternal === 'function') {
      event.preventDefault()
      void window.qwicksGui.openExternal(href).catch(() => undefined)
    }
  }
  ...
  const handleDoubleClick = (event: MouseEvent<HTMLAnchorElement>): void => {
    ...
    void openWorkspacePathInEditor(resolvedFileTarget, workspaceRoot).then((result) => {
      if (!result.ok) {
        void window.qwicksGui?.logError?.('editor-open', 'Failed to open file reference', {
