...
export function useSpeechToTextSettings(): QWicksSpeechToTextSettingsV1 | null {
  const [speechToText, setSpeechToText] = useState<QWicksSpeechToTextSettingsV1 | null>(null)

  useEffect(() => {
    let cancelled = false
    const apply = (settings: AppSettingsV1): void => {
      if (!cancelled) setSpeechToText(resolveQWicksSpeechToTextSettings(settings))
    }
    if (typeof window.qwicksGui?.getSettings === 'function') {
      void window.qwicksGui.getSettings().then(apply).catch(() => undefined)
    }
...
export function useVoiceDictation({
  ...
  speechToText?: QWicksSpeechToTextSettingsV1 | null
}): {
  ...
  const transcribeBlob = useCallback(async (blob: Blob, durationMs: number, intent: VoiceDictationIntent): Promise<void> => {
    try {
      ...
      const result = await window.qwicksGui.transcribeSpeech({
