...
import type { ClawImInstallQrResult } from '@shared/qwicks-gui-api'
...
export function ClawAddImDialog({
  ...
  useEffect(() => {
    let cancelled = false
    if (typeof window.qwicksGui?.getSettings !== 'function') return
  ...
  const startOfficialInstallQr = async (): Promise<void> => {
    ...
    if (typeof window.qwicksGui?.startClawImInstallQr !== 'function') {
      setInstallQr({
        status: 'error',
        url: '',
        deviceCode: '',
    ...
    try {
      result = await window.qwicksGui.startClawImInstallQr(officialInstallProvider, {
    ...
    const waitForInstall = async (): Promise<void> => {
      try {
        const poll = await window.qwicksGui.pollClawImInstall(officialInstallProvider, result.deviceCode)
        if (installAttempt !== installAttemptRef.current) return
        if (poll.done) {
          clearInstallTimers()
