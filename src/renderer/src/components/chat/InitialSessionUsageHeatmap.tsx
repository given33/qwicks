...
export function InitialSessionUsageHeatmapView({
  ...
  useEffect(() => {
    let cancelled = false
    if (typeof window === 'undefined' || typeof window.qwicksGui?.getSettings !== 'function') return
    void window.qwicksGui.getSettings()
      .then((settings) => {
        if (!cancelled) setModelLabel(settings.agents.qwicks.model.trim())
      })
      .catch(() => {
