...
export function ChatFileTreePanel({
  ...
  const loadDirectory = useCallback((path: string): void => {
    if (!root || typeof window.qwicksGui?.listWorkspaceDirectory !== 'function') return
  ...
  const revealEntry = async (entry: WorkspaceEntry): Promise<void> => {
    if (typeof window.qwicksGui?.openEditorPath !== 'function') return
    await window.qwicksGui.openEditorPath({
      path: entry.path,
      workspaceRoot: root,
        ...
        >
          ...
          >
            ...
            <span className="min-w-0 truncate">
              {window.qwicksGui?.platform === 'darwin'
