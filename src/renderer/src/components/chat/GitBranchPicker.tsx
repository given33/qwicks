...
export function GitBranchPicker({ workspaceRoot }: Props): ReactElement | null {
  ...
  const load = useCallback(async (): Promise<void> => {
    if (!root || typeof window.qwicksGui?.getGitBranches !== 'function') return
    setLoading(true)
    setError(null)
    try {
      const next = await window.qwicksGui.getGitBranches(root)
      setResult(next)
  ...
  const checkoutBranchWorktree = async (branch: string): Promise<void> => {
    ...
    setError(null)
    try {
      const next = await window.qwicksGui.checkoutGitBranchWorktree(root, branch)
      setResult(next)
      if (!next.ok) {
  ...
  const createBranch = async (): Promise<void> => {
    ...
    setError(null)
    try {
      const next = await window.qwicksGui.createGitBranchWorktree(root, branch)
      setResult(next)
      if (!next.ok) {
        setError(next.message)
        return
