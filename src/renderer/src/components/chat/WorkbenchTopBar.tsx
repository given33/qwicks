...
export function WorkbenchTopBar({
  ...
  useEffect(() => {
    let cancelled = false
    if (typeof window.qwicksGui?.listEditors !== 'function') return

    void window.qwicksGui.listEditors()
      .then((result) => {
        if (cancelled) return
  ...
  useEffect(() => {
    if (typeof window.qwicksGui?.onGuiUpdateState !== 'function') return
    const applyState = (state: GuiUpdateState): void => {
      setGuiUpdateState(state)
    }
    const unsubscribe = window.qwicksGui.onGuiUpdateState(applyState)
    if (typeof window.qwicksGui?.getGuiUpdateState === 'function') {
      void window.qwicksGui.getGuiUpdateState().then(applyState).catch(() => undefined)
    }
    return unsubscribe
  }, [])
  ...
  const runGuiUpdateAction = async (): Promise<void> => {
    ...
    if (
      typeof window.qwicksGui?.downloadGuiUpdate !== 'function' ||
      typeof window.qwicksGui?.installGuiUpdate !== 'function'
    ) {
      return
    }
    ...
    try {
      if (!guiUpdateAction.downloaded && guiUpdateState.status !== 'downloaded') {
        const downloadResult = await window.qwicksGui.downloadGuiUpdate(guiUpdateAction.channel)
        if (!downloadResult.ok) return
      }
      const installResult = await window.qwicksGui.installGuiUpdate()
      if (!installResult.ok && typeof window.qwicksGui?.logError === 'function') {
        await window.qwicksGui.logError('gui-update', 'Failed to install GUI update from workbench top bar', {
          version: guiUpdateAction.latestVersion,
          message: installResult.message
        })
      }
    } catch (error) {
      if (typeof window.qwicksGui?.logError === 'function') {
        await window.qwicksGui.logError('gui-update', 'Failed to apply GUI update from workbench top bar', {
          version: guiUpdateAction.latestVersion,
