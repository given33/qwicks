...
import type { LegacySessionDetectResult } from '@shared/qwicks-gui-api'
...
export function LegacySessionImportCard({
  ...
  const refreshDetection = useCallback(async () => {
    if (typeof window.qwicksGui?.detectLegacySessions !== 'function') {
      setDetecting(false)
      return
    }
    setDetecting(true)
    try {
      setDetection(await window.qwicksGui.detectLegacySessions())
    } catch (error) {
  ...
  const runImport = useCallback(
    async (sourceDir?: string) => {
      if (typeof window.qwicksGui?.importLegacySessions !== 'function') return
      setBusy(true)
      setNotice(null)
      try {
        const result = await window.qwicksGui.importLegacySessions(sourceDir)
        if (!result.ok) {
        ...
        if (result.imported > 0 && typeof window.qwicksGui?.confirmDialog === 'function') {
          const restart = await window.qwicksGui.confirmDialog({
          ...
          if (restart && typeof window.qwicksGui?.restartRuntime === 'function') {
            setRestarting(true)
            try {
              await window.qwicksGui.restartRuntime()
            } finally {
              setRestarting(false)
            }
          }
  ...
  const pickAndImport = useCallback(async () => {
    if (typeof window.qwicksGui?.pickLegacySessionDir !== 'function') return
    try {
      const picked = await window.qwicksGui.pickLegacySessionDir()
