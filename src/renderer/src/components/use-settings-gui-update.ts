...
export function useSettingsGuiUpdate({
  ...
  category: 'general' | 'modelConfiguration' | 'providers' | 'write' | 'imageGeneration' | 'mediaGeneration' | 'speechToText' | 'agents' | 'skills' | 'mcp' | 'archives' | 'permissions' | 'worktree' | 'memory' | 'shortcuts' | 'easterEgg' | 'claw' | 'updates' | 'debug'
  ...
  const checkGuiUpdate = useCallback(async (): Promise<void> => {
    if (typeof window.qwicksGui?.checkGuiUpdate !== 'function') return
    setCheckingGuiUpdate(true)
    setGuiUpdateError(null)
    try {
      const info = await window.qwicksGui.checkGuiUpdate(channel)
  ...
  const downloadGuiUpdate = async (): Promise<void> => {
    if (typeof window.qwicksGui?.downloadGuiUpdate !== 'function') return
    setDownloadingGuiUpdate(true)
    setGuiUpdateProgress(null)
    setGuiUpdateError(null)
    try {
      const result = await window.qwicksGui.downloadGuiUpdate(form?.guiUpdate?.channel)
      if (!result.ok) {
  ...
  const installGuiUpdate = async (): Promise<void> => {
    if (typeof window.qwicksGui?.installGuiUpdate !== 'function') return
    setInstallingGuiUpdate(true)
    setGuiUpdateError(null)
    try {
      const result = await window.qwicksGui.installGuiUpdate()
      if (!result.ok) {
        setGuiUpdateError(result.message)
        setInstallingGuiUpdate(false)
      }
  ...
  useEffect(() => {
    ...
    const unsubscribe = window.qwicksGui.onGuiUpdateState(applyGuiUpdateState)
    if (typeof window.qwicksGui?.getGuiUpdateState === 'function') {
      void window.qwicksGui.getGuiUpdateState().then(applyGuiUpdateState).catch(() => undefined)
    }
    return unsubscribe
  }, [applyGuiUpdateState])
