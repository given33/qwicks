...
import { loadQWicksDiagnostics } from '../lib/load-qwicks-diagnostics'
...
export function SettingsView(): ReactElement {
  ...
  const formQWicks = form ? getQWicksRuntimeSettings(form) : null
  const formPort = formQWicks?.port
  const formGuiUpdateChannel = form?.guiUpdate?.channel
  const formCursorSpotlight = form?.cursorSpotlight
  const settingsPlatform = typeof window !== 'undefined' ? window.qwicksGui?.platform ?? '' : ''
  const settingsHomeDir = typeof window !== 'undefined' ? window.qwicksGui?.homeDir ?? '' : ''
  ...
  useEffect(() => {
    let cancelled = false
    if (typeof window.qwicksGui === 'undefined') {
  ...
  const loadWriteDebugEntries = useCallback(async (): Promise<void> => {
    ...
    try {
      const completionEntries = typeof window.qwicksGui?.listWriteInlineCompletionDebugEntries === 'function'
        ? await window.qwicksGui.listWriteInlineCompletionDebugEntries()
        : []
  ...
  useEffect(() => {
    ...
    if (!getActiveAgentApiKey(form).trim()) {
      setCategory('modelConfiguration')
    }
  }, [form])
  ...
  useEffect(() => {
    ...
    if (settingsSection === 'modelConfiguration') {
      setCategory('modelConfiguration')
      return
    }
  ...
  const refreshSkillRoots = useCallback(async (): Promise<void> => {
    if (typeof window.qwicksGui?.listSkillRoots !== 'function') return
    setSkillRootsLoading(true)
    try {
      const workspaceRoot = normalizeWorkspaceRoot(expandHomePath(formWorkspaceRoot ?? ''))
      const result = await window.qwicksGui.listSkillRoots(workspaceRoot || undefined)
  ...
  const loadMcpConfig = async (): Promise<void> => {
    ...
    setMcpNotice(null)
    try {
      const config = await window.qwicksGui.getQWicksConfigFile()
  ...
  const openSkillRoot = async (path: string): Promise<void> => {
    ...
    if (typeof window.qwicksGui?.openSkillRoot !== 'function') return
    setSkillNotice(null)
    const result = await window.qwicksGui.openSkillRoot(path)
    if (!result.ok) {
  ...
  const saveMcpConfig = async (): Promise<void> => {
    if (typeof window.qwicksGui?.setQWicksConfigFile !== 'function') return
    setMcpBusy(true)
    setMcpNotice(null)
    try {
      const result = await window.qwicksGui.setQWicksConfigFile(mcpConfigText)
  ...
  const openMcpConfigDir = async (): Promise<void> => {
    if (typeof window.qwicksGui?.openQWicksConfigDir !== 'function') return
    const result = await window.qwicksGui.openQWicksConfigDir()
    if (!result.ok) {
      setMcpNotice({ tone: 'error', message: result.message ?? t('applyFailed') })
    }
  }

  const refreshQWicksDiagnostics = useCallback(async (): Promise<void> => {
    const provider = getProvider()
    setRuntimeDiagnosticsBusy(true)
    setRuntimeDiagnosticsNotice(null)
    try {
      const loaded = await loadQWicksDiagnostics(provider, {
  ...
  useEffect(() => {
    ...
    void refreshQWicksDiagnostics()
  }, [category, refreshQWicksDiagnostics])
  ...
  const persistSettings = async (snapshot: AppSettingsV1, version: number): Promise<void> => {
    ...
    try {
      ...
      setSaveStatus('error')
      void window.qwicksGui?.logError?.('settings', 'Failed to apply settings', { message }).catch(() => undefined)
    }
  }
  ...
  const qwicks = getQWicksRuntimeSettings(form)
  ...
  const updateQWicks = (patch: Partial<AppSettingsV1['agents']['qwicks']>): void => {
    update({ agents: qwicksSettingsPatch(patch) })
  }
  ...
  const pickWorkspace = async (): Promise<void> => {
    try {
      ...
      const picked = await window.qwicksGui.pickWorkspaceDirectory(expandHomePath(form.workspaceRoot) || undefined)
  ...
  const clearWriteDebugEntries = async (): Promise<void> => {
    ...
    setWriteDebugError(null)
    try {
      if (typeof window.qwicksGui?.clearWriteInlineCompletionDebugEntries === 'function') {
        await window.qwicksGui.clearWriteInlineCompletionDebugEntries()
      }
