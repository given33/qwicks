...
const PREVIEW_URL_STORAGE_KEY = 'qwicks.devPreview.url'
const PREVIEW_AUTO_FOLLOW_STORAGE_KEY = 'qwicks.devPreview.autoFollow'
...
export function DevBrowserPanel({
  ...
  const useElectronWebview = canUseElectronWebviewEnvironment({
    openExternalAvailable: typeof window.qwicksGui?.openExternal === 'function',
  ...
  const openExternal = (): void => {
    ...
    if (typeof window.qwicksGui?.openExternal === 'function') {
      void window.qwicksGui.openExternal(normalized).catch(() => undefined)
      return
    }
      ...
      <div className="relative min-h-0 flex-1 bg-white dark:bg-ds-canvas">
        {!activeUrl ? (
          ...
          <webview
            ...
            ref={webviewRef}
            src={activeUrl}
            partition="persist:qwicks-dev-browser"
