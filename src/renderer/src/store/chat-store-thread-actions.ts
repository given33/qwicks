...
async function ensureRuntimeProviderForSend(input: {
  ...
  try {
    await window.qwicksGui.saveSettingsSilent({ agents: { qwicks: { providerId, model } } })
    ...
    try {
      ...
      if (options.useWorktreePool) {
        try {
          ...
          if (!branch) {
            const branches = await window.qwicksGui.getGitBranches(workspaceRoot)
            if (branches.ok) branch = branches.currentBranch ?? ''
          }
          if (!branch) {
            throw new Error(i18n.t('common:worktreeBranchRequired'))
          }
          const wt = await window.qwicksGui.checkoutGitBranchWorktree(workspaceRoot, branch)
          if (!wt.ok) {
      ...
      // QWicks thread may not be listed until the first message is written.
  ...
  sendMessage: async (text, mode, overrides) => {
    ...
    if (!activeThreadId) {
      try {
        ...
        void window.qwicksGui.logError('create-thread', 'Failed to create thread', {
    ...
    try {
      ...
      if (checkpointWorkspaceRoot && typeof window.qwicksGui.createGitCheckpoint === 'function') {
        const checkpoint = await window.qwicksGui.createGitCheckpoint({
        ...
        if (checkpoint.ok) {
          ...
          void window.qwicksGui.logError('git-checkpoint', 'Failed to create Git checkpoint', {
      ...
      if (channel && typeof window.qwicksGui?.mirrorClawChannelMessage === 'function') {
        const userMirror = await window.qwicksGui.mirrorClawChannelMessage(
          activeThreadId,
          trimmedText,
          'user'
        )
      ...
      clearBusyWatchdog()
      void window.qwicksGui.logError('send-message', 'Failed to send message', {
        message: e instanceof Error ? e.message : String(e),
        threadId: activeThreadId
      }).catch(() => undefined)
