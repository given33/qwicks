...
function notifyTurnComplete(threadId: string | null, state: ChatState, dedupeKey: string): void {
  if (!threadId || typeof window.qwicksGui?.showTurnCompleteNotification !== 'function') return
  ...
  void window.qwicksGui
    ...
    .then((result) => {
      if (result.ok || typeof window.qwicksGui?.logError !== 'function') return
      void window.qwicksGui.logError('notification', 'Turn completion notification failed', {
        message: result.message,
        threadId
      }).catch(() => undefined)
    })
    .catch((error: unknown) => {
      if (typeof window.qwicksGui?.logError !== 'function') return
      void window.qwicksGui.logError('notification', 'Turn completion notification failed', {
...
function releaseThreadWorktreeIfNeeded(threadId: string | null): void {
  ...
  if (typeof window.qwicksGui?.releaseWorktree !== 'function') return
...
export function buildThreadEventSink(
  ...
  return {
    onSeq: (seq) => {
      ...
      // heartbeat (qwicks events route) — so a healthy turn always keeps the
    ...
    onTurnComplete: () => {
      ...
      if (pendingMirror && assistantMirrorText && typeof window.qwicksGui?.mirrorClawChannelMessage === 'function') {
        void window.qwicksGui.mirrorClawChannelMessage(
