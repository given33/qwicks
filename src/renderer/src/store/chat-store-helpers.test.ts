...
import type { ModelProviderModelGroup } from '@shared/qwicks-gui-api'
...
const TURN_MODEL_STORAGE_KEY = 'qwicks.turnModelLabel'
    ...
    conversations: [
      {
        ...
        senderId: 'sender-1',
        senderName: 'Alex',
        localThreadId: 'qwicks-conversation',
        workspaceRoot: '/Users/zxy/project',
        createdAt: now,
        updatedAt: now
      }
...
describe('chat-store Claw helpers', () => {
  ...
  it('collects channel and conversation thread ids for Claw sessions', () => {
    ...
    expect(ids.has('qwicks-channel')).toBe(true)
    expect(ids.has('qwicks-conversation')).toBe(true)
  })
  ...
  it('recognizes Claw managed prompt summaries as Claw sessions', () => {
    ...
    expect(isClawThread({ id: 'qwicks-leaked', title: '[Claw:Feishu Agent]' })).toBe(true)
  })

  it('recognizes Claw sessions by registered thread id', () => {
    expect(
      isClawThread(
        { id: 'qwicks-conversation', title: 'hi' },
        [clawChannel()]
      )
    ).toBe(true)
  })
