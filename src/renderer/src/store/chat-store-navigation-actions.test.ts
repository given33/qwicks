...
function buildHarness(overrides?: {
  ...
  let state = {
    ...
    busy: false,
    clawChannels: [],
    codeWorkspaceRoots: ['~/.qwicks/default_workspace'],
    composerPickList: [],
    createThread,
    currentTurnId: null,
    ...
    threads: [
      thread({
        ...
        workspace: '~/.qwicks/default_workspace'
      })
    ],
    unreadThreadIds: {},
    watchTurnCompletion: {},
...
describe('chat-store navigation workspace selection', () => {
  ...
  it('does not move the only default thread into a newly picked empty workspace', async () => {
    ...
    expect(pickWorkspaceDirectory).toHaveBeenCalledWith('~/.qwicks/default_workspace')
  ...
  it('selectWorkspaceRoot persists the directory and lands on a clean new conversation', async () => {
    ...
    vi.stubGlobal('window', { qwicksGui: { setSettings } })
    const harness = buildHarness()
...
describe('onClawChannelActivity routes through subscribeThreadEventsLive (not selectThread)', () => {
  ...
  it('calls subscribeThreadEventsLive when activeThreadId differs from the bot thread', async () => {
    ...
    // Capture the callback registered via window.qwicksGui.onClawChannelActivity
    ...
    const getSettings = vi.fn(async () => ({
      workspaceRoot: '~/.qwicks/default_workspace',
      write: {
        defaultWorkspaceRoot: '~/.qwicks/default_workspace',
        activeWorkspaceRoot: '~/.qwicks/default_workspace',
        workspaces: []
      },
      claw: {
        channels: [
      ...
      uiFontScale: 1,
      locale: 'en',
      agents: { qwicks: { apiKey: 'test-key', model: 'deepseek-v4-pro', baseUrl: '' } },
      disabledSkillIds: []
    }))
    vi.stubGlobal('window', {
      qwicksGui: {
