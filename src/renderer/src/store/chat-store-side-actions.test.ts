...
describe('chat-store-side-actions', () => {
  ...
  it('uses the QWicks default model when side creation has no parent or composer model to inherit', async () => {
    ...
    expect(state.sideConversations[id!].model).toBe(DEFAULT_QWICKS_MODEL)
  })
  ...
  it('promoteSideConversation clears the relation by PATCH /v1/threads/{id} and refreshes the thread list', async () => {
    const { actions, state } = buildHarness()
    const id = (await actions.spawnSideConversation())!
    const runtimeRequest = globalThis.window.qwicksGui.runtimeRequest as ReturnType<typeof vi.fn>
