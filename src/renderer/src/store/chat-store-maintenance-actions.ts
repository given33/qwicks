...
export function createMaintenanceActions(
  ...
  deleteThread: async (threadId) => {
    ...
    if (wtRecord?.poolIndex !== undefined) {
      try {
        await window.qwicksGui.releaseWorktree({
  ...
  rewindAndResend: async (userBlockId, newText) => {
    ...
    if (checkpointId) {
      const restored = await window.qwicksGui.restoreGitCheckpoint({ checkpointId }).catch((error) => ({
        ok: false as const,
  ...
  resolveApproval: async (blockId, decision) => {
    ...
    try {
      ...
      void window.qwicksGui.logError('approval', 'Failed to submit approval decision', {
        message: msg,
        blockId
      }).catch(() => undefined)
      set((s) => ({
  ...
  resolveUserInput: async (blockId, action) => {
    ...
    try {
      ...
      const msg = formatRuntimeError(e)
      void window.qwicksGui.logError('user-input', 'Failed to resolve user input', {
        message: msg,
        blockId
      }).catch(() => undefined)
      set((s) => ({
  ...
  interrupt: async (options) => {
    ...
    try {
      ...
      const msg = formatRuntimeError(e)
      void window.qwicksGui.logError('interrupt', 'Failed to interrupt turn', { message: msg }).catch(() => undefined)
      set({
        error: msg,
        ...(shouldOpenSettingsForError(e)
          ? { route: 'settings' as const, settingsSection: 'agents' as const }
