...
> {
  ...
  return {
    ...
    setComposerModel: (modelId, providerId) => {
      ...
      if (!activeThreadId && trimmed && trimmed.toLowerCase() !== 'auto' && typeof window.qwicksGui !== 'undefined') {
        void window.qwicksGui.saveSettingsSilent({ agents: { qwicks: { model: trimmed } } })
      }
    },
    ...
    loadComposerModels: async () => {
      ...
      const task = (async () => {
        const res = await window.qwicksGui.fetchUpstreamModels()
