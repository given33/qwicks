...
describe('chat-store-thread-actions queued messages', () => {
  ...
  it('does not queue GUI plan messages while another turn is active', async () => {
    ...
    const guiPlan: GuiPlanMessageContext = {
      ...
      relativePath: '.qwickssdd/plan/feature.md',
      planId: 'plan-1',
    ...
    state.queuedMessages = [
      {
        ...
        guiPlan: {
          operation: 'draft',
          workspaceRoot: '/workspace/deepseek-gui',
          relativePath: '.qwickssdd/plan/one.md',
          planId: 'plan-1'
        }
      },
      {
  ...
  it('applies the selected composer provider before sending a turn', async () => {
    ...
    const saveSettingsSilent = vi.fn(async () => ({
      agents: { qwicks: { providerId: 'xiaomi-token-plan', model: 'mimo-v2.5' } },
      codePromptPrefix: ''
    }))
    ...
    vi.stubGlobal('window', {
      qwicksGui: {
        getSettings: vi.fn(async () => ({
          agents: { qwicks: { providerId: 'minimax-token-plan', model: 'MiniMax-M2' } },
          codePromptPrefix: ''
        })),
        saveSettingsSilent,
        restartRuntime,
  ...
  it('applies an override provider before sending', async () => {
    ...
    registryMock.getProvider.mockReturnValue(provider)
    const saveSettingsSilent = vi.fn(async () => ({
      agents: { qwicks: { providerId: 'minimax-token-plan', model: 'MiniMax-M3' } },
      codePromptPrefix: ''
    }))
    const restartRuntime = vi.fn(async () => undefined)
    vi.stubGlobal('window', {
      qwicksGui: {
        getSettings: vi.fn(async () => ({
          agents: { qwicks: { providerId: 'deepseek', model: 'deepseek-v4-pro' } },
          codePromptPrefix: ''
        })),
        saveSettingsSilent,
        restartRuntime,
