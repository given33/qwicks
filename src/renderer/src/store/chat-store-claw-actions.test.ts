...
describe('chat-store Claw actions helpers', () => {
  ...
  it('uses the channel threadId when the latest conversation has none', () => {
    const item = channel({ threadId: 'qwicks-channel-thread' })
    const conversation = { ...item.conversations[0], localThreadId: '' }
    expect(clawThreadIdForProvider(item, conversation)).toBe('qwicks-channel-thread')
  })

  it('recovers an unmapped Claw managed QWicks session before creating a new empty one', () => {
    const item = channel()
  ...
  it('writes recovered provider thread ids back to both channel and conversation', () => {
    ...
    const next = channelWithClawThreadMapping(channel(), 'qwicks-thread', now, 'conversation-1')

    expect(next.threadId).toBe('qwicks-thread')
    expect(next.conversations[0]?.localThreadId).toBe('qwicks-thread')
  })
  ...
  it('keeps an empty IM channel on the Claw route instead of selecting a stale missing thread', async () => {
    ...
    vi.stubGlobal('window', { qwicksGui })
    ...
    expect(qwicksGui.setSettings).toHaveBeenCalledWith({
      claw: {
