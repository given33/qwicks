...
const COMPOSER_MODEL_STORAGE_KEY = 'qwicks.composerModel'
const COMPOSER_PROVIDER_STORAGE_KEY = 'qwicks.composerProviderId'
const THREAD_COMPOSER_SELECTION_STORAGE_KEY = 'qwicks.threadComposerSelection.v1'
...
describe('chat-store app actions composer model loading', () => {
  ...
  it('updates the composer provider when the picker supplies a provider id', () => {
    ...
    expect(state.composerProviderId).toBe('minimax')
    expect(localStorage.getItem(COMPOSER_PROVIDER_STORAGE_KEY)).toBe('minimax')
    expect(window.qwicksGui.saveSettingsSilent).toHaveBeenCalledWith({
      agents: { qwicks: { model: 'MiniMax-M2' } }
    })
  })

  it('keeps active-thread model changes out of the global QWicks default', () => {
    const { actions, state } = buildHarness({
      ok: true,
      modelIds: ['MiniMax-M2'],
      defaultModelId: 'deepseek-v4-pro',
    ...
    expect(window.qwicksGui.saveSettingsSilent).not.toHaveBeenCalled()
  })
  ...
  it('allows switching an empty chat from vision to text-only', () => {
    ...
    expect(window.qwicksGui.saveSettingsSilent).toHaveBeenCalledWith({
      agents: { qwicks: { model: 'text-model' } }
    })
  })
