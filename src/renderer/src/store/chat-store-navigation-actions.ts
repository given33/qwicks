...
export function createNavigationActions(
...
): Pick<ChatState, 'openCode' | 'probeRuntime' | 'boot' | 'chooseWorkspace' | 'selectWorkspaceRoot' | 'clearWorkspace' | 'deleteWorkspace' | 'refreshThreads' | 'setThreadSearch' | 'setShowArchivedThreads'> {
  return {
  openCode: async () => {
    const state = get()
  ...
  probeRuntime: async (mode = 'user', options) => {
    ...
    try {
      if (typeof window.qwicksGui === 'undefined') {
        throw new Error(
          'Preload bridge missing (window.qwicksGui). Restart the app or check BrowserWindow preload path.'
        )
      }
      ...
      try {
        if (typeof window.qwicksGui === 'undefined') {
          set({
            ...
            runtimeConnection: 'offline',
            runtimeErrorDetail: 'Preload bridge missing (window.qwicksGui). Restart the app or check BrowserWindow preload path.',
        ...
        if (!runtimeStatusUnsubscribe && typeof window.qwicksGui.onRuntimeStatus === 'function') {
          runtimeStatusUnsubscribe = window.qwicksGui.onRuntimeStatus((status) => {
        ...
        if (!trayActionUnsubscribe && typeof window.qwicksGui.onTrayAction === 'function') {
          trayActionUnsubscribe = window.qwicksGui.onTrayAction((action) => {
            set({ route: 'chat' })
        ...
        if (!clawChannelActivityUnsubscribe && typeof window.qwicksGui.onClawChannelActivity === 'function') {
          clawChannelActivityUnsubscribe = window.qwicksGui.onClawChannelActivity(({ channelId, threadId }) => {
            void (async () => {
              const state = get()
              if (typeof window.qwicksGui === 'undefined') return
              const settings = await rendererRuntimeClient.getSettings({ forceRefresh: true })
  ...
  chooseWorkspace: async ({ createThreadAfter = false, selectThreadAfter = true } = {}) => {
    try {
      if (typeof window.qwicksGui === 'undefined' || typeof window.qwicksGui.pickWorkspaceDirectory !== 'function') {
        throw new Error(i18n.t('common:workspacePickerUnavailable'))
      }
      const picked = await window.qwicksGui.pickWorkspaceDirectory(get().workspaceRoot || undefined)
  ...
  clearWorkspace: async () => {
    try {
      if (typeof window.qwicksGui === 'undefined' || typeof window.qwicksGui.setSettings !== 'function') {
        return
      }
  ...
  deleteWorkspace: async (workspacePath) => {
    ...
    try {
      ...
      if (normalizeWorkspaceRoot(get().workspaceRoot) === normalizedPath) {
        try {
          if (typeof window.qwicksGui?.setSettings === 'function') {
            const next = await rendererRuntimeClient.setSettings({ workspaceRoot: '' })
            set({
              workspaceRoot: normalizeWorkspaceRoot(next.workspaceRoot),
              codeWorkspaceRoots: get().codeWorkspaceRoots,
  ...
  refreshThreads: async () => {
    ...
    try {
      ...
      // Preserve the active QWicks thread when it is not in the listing yet.
