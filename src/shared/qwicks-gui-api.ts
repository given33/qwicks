...
export type QWicksRuntimeStatusPayload = {
...
export type LegacySessionSourceKind = 'qwicks' | 'coreagent' | 'custom'
...
export type LegacySessionDetectResult = {
  /** Destination threads directory (current QWicks data dir + /threads). */
  destDir: string
...
export type QWicksGuiApi = {
  ...
  getQWicksConfigFile: () => Promise<DeepseekConfigFileResult>
  setQWicksConfigFile: (content: string) => Promise<DeepseekConfigSaveResult>
  openQWicksConfigDir: () => Promise<PathOpenResult>
  ...
  onRuntimeStatus: (handler: (payload: QWicksRuntimeStatusPayload) => void) => () => void
