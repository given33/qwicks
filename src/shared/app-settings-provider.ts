...
import { normalizeModelEndpointFormat, type ModelEndpointFormat } from '../../qwicks/src/contracts/model-endpoint-format.js'
import { getQWicksRuntimeSettings } from './app-settings-qwicks'
...
type QWicksMediaSettingCore = Partial<{
  enabled: boolean
  providerId: string
  baseUrl: string
  apiKey: string
...
export function defaultMiniMaxMediaGenerationQWicksPatch(input: {
  providers: readonly ModelProviderProfileV1[]
  currentQWicks?: Partial<QWicksRuntimeSettingsV1>
  qwicksPatch?: QWicksRuntimeSettingsPatchV1
}): QWicksRuntimeSettingsPatchV1 | undefined {
  const patch: QWicksRuntimeSettingsPatchV1 = {}
  if (!input.qwicksPatch?.textToSpeech && isBlankQWicksMediaSetting(input.currentQWicks?.textToSpeech)) {
    const match = configuredMiniMaxMediaCapability(input.providers, 'textToSpeech', input.currentQWicks?.providerId)
    if (match) {
      patch.textToSpeech = {
        enabled: true,
  ...
  if (!input.qwicksPatch?.musicGeneration && isBlankQWicksMediaSetting(input.currentQWicks?.musicGeneration)) {
    const match = configuredMiniMaxMediaCapability(input.providers, 'music', input.currentQWicks?.providerId)
    if (match) {
  ...
  if (!input.qwicksPatch?.videoGeneration && isBlankQWicksMediaSetting(input.currentQWicks?.videoGeneration)) {
    const match = configuredMiniMaxMediaCapability(input.providers, 'video', input.currentQWicks?.providerId)
    if (match) {
...
function isBlankQWicksMediaSetting(setting: QWicksMediaSettingCore | undefined): boolean {
...
export function resolveQWicksMusicGenerationSettings(settings: AppSettingsV1): QWicksMusicGenerationSettingsV1 {
  const runtime = getQWicksRuntimeSettings(settings)
...
export function resolveQWicksVideoGenerationSettings(settings: AppSettingsV1): QWicksVideoGenerationSettingsV1 {
  const runtime = getQWicksRuntimeSettings(settings)
...
export function resolveQWicksMemoryEnabled(settings: AppSettingsV1): boolean {
  const runtime = getQWicksRuntimeSettings(settings)
...
export function resolveQWicksImageGenerationSettings(settings: AppSettingsV1): QWicksImageGenerationSettingsV1 {
  const runtime = getQWicksRuntimeSettings(settings)
...
export function resolveQWicksRuntimeSettings(settings: AppSettingsV1): QWicksRuntimeSettingsV1 {
  const runtime = getQWicksRuntimeSettings(settings)
  ...
  return {
    ...
    // settings-apply gate then stops a perfectly healthy QWicks runtime.
    ...
    memoryEnabled: resolveQWicksMemoryEnabled(settings)
  }
}
