...
describe('qwicks defaults', () => {
  it('keeps a single shared default data directory source', () => {
    expect(defaultQWicksRuntimeSettings().dataDir).toBe(DEFAULT_QWICKS_DATA_DIR)
  })

  it('defaults the assistant model to v4 pro', () => {
    expect(defaultQWicksRuntimeSettings().model).toBe(DEFAULT_QWICKS_MODEL)
  })

  it('defaults approval policy to auto', () => {
    expect(defaultQWicksRuntimeSettings().approvalPolicy).toBe(DEFAULT_APPROVAL_POLICY)
    expect(defaultQWicksRuntimeSettings().approvalPolicy).toBe('auto')
  })

  it('defaults sandbox mode to full access', () => {
    expect(defaultQWicksRuntimeSettings().sandboxMode).toBe(DEFAULT_SANDBOX_MODE)
    expect(defaultQWicksRuntimeSettings().sandboxMode).toBe('danger-full-access')
  })

  it('defaults token economy mode to off', () => {
    expect(defaultQWicksRuntimeSettings().tokenEconomyMode).toBe(false)
    expect(defaultQWicksRuntimeSettings().tokenEconomy).toMatchObject({
      enabled: false,
  ...
  it('defaults MCP search discovery to off', () => {
    expect(defaultQWicksRuntimeSettings().mcpSearch).toMatchObject({
      enabled: false,
      mode: 'auto',
  ...
  it('defaults image generation to off with empty provider fields', () => {
    expect(defaultQWicksRuntimeSettings().imageGeneration).toEqual({
      enabled: false,
      providerId: '',
  ...
  it('defaults media generation to off with empty provider fields', () => {
    expect(defaultQWicksRuntimeSettings().textToSpeech).toEqual({
      enabled: false,
      providerId: '',
    ...
    expect(defaultQWicksRuntimeSettings().musicGeneration).toEqual({
      enabled: false,
      providerId: '',
    ...
    expect(defaultQWicksRuntimeSettings().videoGeneration).toEqual({
      enabled: false,
      providerId: '',
...
describe('isQWicksRuntimeInsecure', () => {
...
describe('mergeQWicksRuntimeSettings', () => {
  it('merges a direct qwicks patch without the envelope wrapper', () => {
  ...
  it('deep-merges token economy settings and keeps the legacy switch synced', () => {
    ...
    const legacySwitch = mergeQWicksRuntimeSettings(next, { tokenEconomyMode: false })
  ...
  it('deep-merges advanced QWicks settings', () => {
    const current = defaultQWicksRuntimeSettings()
    const next = mergeQWicksRuntimeSettings(current, {
      storage: {
        sqlitePath: ' /tmp/qwicks.sqlite3 '
      },
      contextCompaction: {
    ...
    expect(next.storage.sqlitePath).toBe('/tmp/qwicks.sqlite3')
  ...
  it('normalizes the stream idle timeout (0 disables, out-of-range clamps)', () => {
    ...
    const set = mergeQWicksRuntimeSettings(current, {
    ...
    expect(
      mergeQWicksRuntimeSettings(current, { runtimeTuning: { streamIdleTimeoutMs: 0 } })
        .runtimeTuning.streamIdleTimeoutMs
    ).toBe(0)

    // Negative falls back to the default; absurdly large clamps to the cap.
    expect(
      mergeQWicksRuntimeSettings(current, { runtimeTuning: { streamIdleTimeoutMs: -5 } })
        .runtimeTuning.streamIdleTimeoutMs
    ).toBe(45000)
    expect(
      mergeQWicksRuntimeSettings(current, { runtimeTuning: { streamIdleTimeoutMs: 999_999_999 } })
        .runtimeTuning.streamIdleTimeoutMs
    ).toBe(3_600_000)
  })
  ...
  it('deep-merges image generation settings and normalizes invalid values', () => {
    ...
    const sized = mergeQWicksRuntimeSettings(next, {
    ...
    const invalidSize = mergeQWicksRuntimeSettings(sized, {
  ...
  it('deep-merges media generation settings and normalizes invalid values', () => {
    ...
    const invalid = mergeQWicksRuntimeSettings(next, {
  ...
  it('defaults missing MiniMax media generation settings to the configured MiniMax provider', () => {
    ...
    const normalized = normalizeAppSettings({
      ...
      agents: { qwicks: legacyQWicks as AppSettingsV1['agents']['qwicks'] }
    })
    const resolved = resolveQWicksRuntimeSettings(normalized)

    expect(normalized.agents.qwicks.textToSpeech).toEqual(expect.objectContaining({
      enabled: true,
      providerId: 'minimax',
      protocol: 'minimax-t2a',
      model: 'speech-2.8-hd'
    }))
    expect(normalized.agents.qwicks.musicGeneration).toEqual(expect.objectContaining({
      enabled: true,
      providerId: 'minimax',
      protocol: 'minimax-music',
      model: 'music-2.6'
    }))
    expect(normalized.agents.qwicks.videoGeneration).toEqual(expect.objectContaining({
      enabled: true,
      providerId: 'minimax',
      protocol: 'minimax-video',
      model: 'MiniMax-Hailuo-2.3'
...
describe('qwicks envelope helpers', () => {
  it('wraps runtime settings and patches into the compatibility shell', () => {
    const runtime = defaultQWicksRuntimeSettings()
    expect(qwicksSettingsEnvelope(runtime)).toEqual({ qwicks: runtime })
    expect(qwicksSettingsPatch({ model: 'deepseek-reasoner' })).toEqual({
      qwicks: { model: 'deepseek-reasoner' }
    })
  })

  it('applies a qwicks patch onto full app settings', () => {
    const current = settings()
    const next = applyQWicksRuntimePatch(current, { model: 'deepseek-reasoner' })
    expect(next.agents.qwicks.model).toBe('deepseek-reasoner')
    expect(next.write).toEqual(current.write)
  })
})

describe('legacy QWicks defaults migration', () => {
  it('normalizes old master settings without an agents.qwicks envelope', () => {
    const normalized = normalizeAppSettings({
      version: 1,
      locale: 'zh',
      theme: 'dark',
    ...
    expect(normalized.agents.qwicks).toEqual(expect.objectContaining({
      binaryPath: '',
      port: 8787,
      autoStart: false,
      runtimeToken: 'old-token',
  ...
  it('moves the legacy local HTTP default port to the QWicks default port', () => {
    ...
    expect(migrated.agents?.qwicks?.port).toBe(8899)
  })
  ...
  it('fills image generation defaults for settings stored before the feature existed', () => {
    ...
    expect(migrated.agents?.qwicks?.imageGeneration).toEqual({
      enabled: false,
      providerId: '',
      protocol: 'openai-images',
      baseUrl: '',
  ...
  it('uses the current approval policy default for missing legacy local HTTP settings', () => {
    ...
    expect(migrated.agents?.qwicks?.approvalPolicy).toBe(DEFAULT_APPROVAL_POLICY)
  })

  it('upgrades old persisted QWicks defaults to the current defaults', () => {
    const migrated = migrateLegacyAppSettings({
      version: 1,
      agents: {
        qwicks: {
    ...
    expect(migrated.agents?.qwicks).toEqual(expect.objectContaining({
      dataDir: DEFAULT_QWICKS_DATA_DIR,
      model: DEFAULT_QWICKS_MODEL
    }))
  })

  it('preserves a non-legacy QWicks model override', () => {
    const migrated = migrateLegacyAppSettings({
      version: 1,
      agents: {
        qwicks: {
          dataDir: '/tmp/custom-qwicks',
  ...
  it('preserves custom model providers while migrating legacy settings', () => {
    ...
    expect(migrated.agents.qwicks.providerId).toBe('custom-provider-2')
    expect(resolveQWicksRuntimeSettings(migrated)).toEqual(
      expect.objectContaining({
        apiKey: 'sk-custom',
        baseUrl: 'https://custom.example/v1',
        endpointFormat: 'responses'
...
describe('claw runtime prompts', () => {
  ...
  it('tells Claw agents to use the image tool when image generation is configured', () => {
    const state = settings()
    state.agents.qwicks.imageGeneration = {
      enabled: true,
      providerId: '',
      protocol: 'openai-images',
  ...
  it('tells Claw agents to use media tools when media generation is configured', () => {
    const state = settings()
    state.agents.qwicks.textToSpeech = {
      enabled: true,
      providerId: '',
      protocol: 'minimax-t2a',
      baseUrl: 'https://api.minimax.io',
    ...
    state.agents.qwicks.musicGeneration = {
      enabled: true,
      providerId: '',
      protocol: 'minimax-music',
      baseUrl: 'https://api.minimax.io',
    ...
    state.agents.qwicks.videoGeneration = {
      enabled: true,
      providerId: '',
      protocol: 'minimax-video',
      baseUrl: 'https://api.minimax.io',
...
describe('write inline completion runtime config', () => {
  ...
  it('falls back to the qwicks model when write keeps the default inline model', () => {
    const state = settings()
    state.agents.qwicks.model = 'deepseek-chat'
