...
import { getActiveAgentApiKey, getQWicksRuntimeSettings } from './app-settings-qwicks'
...
export function resolveWriteInlineCompletionProviderId(settings: AppSettingsV1): string {
  ...
  return getQWicksRuntimeSettings(settings).providerId?.trim() || DEFAULT_MODEL_PROVIDER_ID
}
...
export function resolveWriteInlineCompletionModel(
  ...
  const runtimeModel = getQWicksRuntimeSettings(settings).model?.trim() ?? ''
