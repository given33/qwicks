...
describe('normalizeUiPluginManifest', () => {
  ...
  it('rejects reserved and malformed ids', () => {
    for (const id of ['default', 'qwicks', 'ON', 'a', 'Has Space', '../x']) {
      const result = normalizeUiPluginManifest({ ...validManifest, id })
      expect(result.ok).toBe(false)
    }
  })

  it('allows the bundled iqwicks id (iQWicks ships as a pre-installed plugin)', () => {
    expect(normalizeUiPluginManifest({ ...validManifest, id: 'iqwicks' }).ok).toBe(true)
  })
