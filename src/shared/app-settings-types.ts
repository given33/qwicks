...
import type { ApprovalPolicy, SandboxMode } from '../../qwicks/src/contracts/policy.js'
import type { ComputerUseMode } from '../../qwicks/src/contracts/capabilities.js'
import type { ModelEndpointFormat } from '../../qwicks/src/contracts/model-endpoint-format.js'
export {
...
} from '../../qwicks/src/contracts/model-endpoint-format.js'
...
export {
  ...
  type ApprovalPolicy,
  type SandboxMode
} from '../../qwicks/src/contracts/policy.js'
...
export const DEFAULT_WRITE_WORKSPACE_ROOT = '~/.qwicks/write_workspace'
export const DEFAULT_QWICKS_DATA_DIR = '~/.qwicks/data'
...
export const DEFAULT_QWICKS_PORT = 8899
...
export type QWicksRuntimeSettingsV1 = {
  binaryPath: string
  port: number
  autoStart: boolean
  ...
  /** Detailed token-saving behavior used when building QWicks model requests. */
  tokenEconomy: QWicksTokenEconomySettingsV1
  /** When true, the runtime skips bearer-token auth. Local dev only. */
  insecure: boolean
  /** GUI-managed MCP progressive discovery/search settings written into QWicks config.json. */
  mcpSearch: QWicksMcpSearchSettingsV1
  /** Persistent store backend used by QWicks. */
  storage: QWicksStorageSettingsV1
  /** Fallback compaction thresholds and summary behavior. Per-model thresholds live in QWicks config models.profiles. */
  contextCompaction: QWicksContextCompactionSettingsV1
  /** Low-level loop guards and model argument repair tuning. */
  runtimeTuning: QWicksRuntimeTuningSettingsV1
  ...
  quality: QWicksDesignQualitySettingsV1
}

/** Detection aggressiveness for the design-quality linter. */
export type QWicksDesignQualityStrictness = 'relaxed' | 'standard' | 'strict'

export type QWicksDesignQualitySettingsV1 = {
  /** Master switch. Off means the builtin design-quality hook never fires. */
  enabled: boolean
  strictness: QWicksDesignQualityStrictness
...
export type QWicksComputerUseSettingsV1 = {
...
export type QWicksImageGenerationSettingsV1 = {
  enabled: boolean
...
export type QWicksTextToSpeechSettingsV1 = {
  enabled: boolean
...
export type QWicksMusicGenerationSettingsV1 = {
  enabled: boolean
...
export type QWicksVideoGenerationSettingsV1 = {
  enabled: boolean
...
export type QWicksMcpSearchMode = 'direct' | 'search' | 'auto'

export type QWicksMcpSearchSettingsV1 = {
  enabled: boolean
  mode: QWicksMcpSearchMode
...
export type QWicksCompactionSummaryMode = 'heuristic' | 'model'

export type QWicksHistoryHygieneSettingsV1 = {
...
export type QWicksTokenEconomySettingsV1 = {
  enabled: boolean
  compressToolDescriptions: boolean
  compressToolResults: boolean
  conciseResponses: boolean
  historyHygiene: QWicksHistoryHygieneSettingsV1
}

export type QWicksContextCompactionSettingsV1 = {
  defaultSoftThreshold: number
  defaultHardThreshold: number
  summaryMode: QWicksCompactionSummaryMode
  summaryTimeoutMs: number
  summaryMaxTokens: number
  summaryInputMaxBytes: number
}

export type QWicksToolStormSettingsV1 = {
  enabled: boolean
  windowSize: number
  threshold: number
}

export type QWicksToolArgumentRepairSettingsV1 = {
  maxStringBytes: number
}

export type QWicksRuntimeTuningSettingsV1 = {
  /**
  ...
  toolStorm: QWicksToolStormSettingsV1
  toolArgumentRepair: QWicksToolArgumentRepairSettingsV1
}

/**
 * Compatibility shell kept because persisted settings still use the
 * `agents.qwicks` envelope. Prefer operating on the contained
 * `QWicksRuntimeSettingsV1` directly in new code.
 */
export type QWicksSettingsEnvelopeV1 = {
  qwicks: QWicksRuntimeSettingsV1
}

/** @deprecated Use `QWicksSettingsEnvelopeV1`. */
export type AgentRuntimeSettingsMapV1 = QWicksSettingsEnvelopeV1

export type QWicksRuntimeTuningSettingsPatchV1 = {
  streamIdleTimeoutMs?: number
  toolStorm?: Partial<QWicksToolStormSettingsV1>
  toolArgumentRepair?: Partial<QWicksToolArgumentRepairSettingsV1>
}

export type QWicksTokenEconomySettingsPatchV1 = Partial<
  Omit<QWicksTokenEconomySettingsV1, 'historyHygiene'>
> & {
  historyHygiene?: Partial<QWicksHistoryHygieneSettingsV1>
}
...
> & {
  ...
  tokenEconomy?: QWicksTokenEconomySettingsPatchV1
  storage?: Partial<QWicksStorageSettingsV1>
...
export type QWicksSettingsEnvelopePatchV1 = {
  qwicks?: QWicksRuntimeSettingsPatchV1
}
...
// reuses the exact same QWicks-runtime execution path as a scheduled task.
// ---------------------------------------------------------------------------
...
export type WorkflowNodeRunResultV1 = {
  ...
  retries?: number
  /** For ai-agent nodes: the QWicks thread it created. */
  threadId: string
  error: string
}
...
export type WorkflowV1 = {
  ...
  name: string
  enabled: boolean
  /** When true, the QWicks agent may invoke this workflow as a tool (list_workflows / run_workflow). */
...
/** The qwicks agent hook phases a workflow can be bound to. Mirrors qwicks's HOOK_PHASES. */
...
/** Binds a Create Loop workflow to a qwicks agent hook phase (reactive automation). */
export type WorkflowHookTriggerV1 = {
  ...
  /** Hook timeout in ms; 0 uses the qwicks default. */
  timeoutMs: number
}
...
export type WorkflowSettingsV1 = {
  enabled: boolean
  defaultWorkspaceRoot: string
  /** Default model provider for new AI nodes. Empty inherits the QWicks runtime provider. */
  providerId?: string
  model: string
  mode: ScheduleRunMode
  keepAwake: boolean
  ...
  webhookPort: number
  /** Optional shared secret required on inbound webhook requests (x-qwicks-secret / Bearer). */
  ...
  /** Workflows bound to qwicks agent hook phases (reactive automation in code mode). */
...
export type ClawImSettingsV1 = {
  ...
  workspaceRoot: string
  /** Default model provider for IM channels without their own provider. Empty inherits QWicks runtime provider. */
  providerId?: string
  model: string
  mode: ClawRunMode
...
export type ClawImTelegramPlatformCredentialV1 = {
  ...
  /** Bot username resolved via getMe, e.g. "my_qwicks_bot". Cosmetic only. */
  botUsername?: string
  createdAt: string
}
...
export type ClawImConversationV1 = {
  ...
  senderId: string
  senderName: string
  /** QWicks thread id this conversation maps to. */
...
export type ClawImChannelV1 = {
  ...
  providerId?: string
  model: string
  /** QWicks thread id this channel maps to. */
  threadId: string
  workspaceRoot: string
...
export type WriteInlineCompletionSettingsV1 = {
  ...
  /** When true, Write inherits QWicks's selected provider instead of using `providerId`. */
  inheritProvider: boolean
  /** Selected provider for Write inline completion when `inheritProvider` is false. */
  providerId: string
  apiKey: string
  baseUrl: string
  /** When true, Write inherits QWicks's runtime model instead of using `model` as an override. */
...
export type AppSettingsV1 = {
  ...
  agents: QWicksSettingsEnvelopeV1
  workspaceRoot: string
  log: LogConfigV1
...
> & {
  ...
  agents?: QWicksSettingsEnvelopePatchV1
