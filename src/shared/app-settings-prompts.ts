...
/**
 * Read the QWicks thread id from a legacy `agentThreadIds` record.
...
export function readLegacyAgentThreadId(input: unknown): string {
  ...
  const candidates = [
    typeof raw.qwicks === 'string' ? raw.qwicks.trim() : '',
...
export function buildClawRuntimePrompt(
  ...
  const imageGeneration = settings.provider && settings.agents
    ? resolveQWicksImageGenerationSettings(settings as AppSettingsV1)
  ...
  const textToSpeech = settings.provider && settings.agents
    ? resolveQWicksTextToSpeechSettings(settings as AppSettingsV1)
  ...
  const musicGeneration = settings.provider && settings.agents
    ? resolveQWicksMusicGenerationSettings(settings as AppSettingsV1)
  ...
  const videoGeneration = settings.provider && settings.agents
    ? resolveQWicksVideoGenerationSettings(settings as AppSettingsV1)
