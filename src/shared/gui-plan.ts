export const GUI_PLAN_RELATIVE_DIR = '.qwickssdd/plan'
...
/**
 * Stable name of the native QWicks plan tool. Kept distinct from the
 * historical `gui_plan_create` MCP bridge so the renderer and QWicks
 ...
 * plan/refine turn so QWicks can scope tool availability to the
 * active plan context.
 */
 ...
 * the public surface the model sees; validation is enforced by QWicks
...
/**
 * Build the deterministic plan id used by both renderer and QWicks.
...
export function validateCreatePlanToolInput(input: Partial<CreatePlanToolInput>): string[] {
  ...
  if (input.plan_relative_path != null) {
    ...
    if (!path) {
      ...
      issues.push('plan_relative_path must be a direct Markdown file under .qwickssdd/plan')
    }
  }
 ...
 * plan path checks. Used by QWicks to verify the active workspace
