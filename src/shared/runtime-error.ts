/**
 * Shared parser for QWicks runtime error bodies.
 *
 * QWicks's contract (`qwicks/src/contracts/errors.ts`) returns
 ...
 * always carries either a QWicks contract code or one of the
 * `LEGACY_MAIN_GUARD_CODES` (main-process guard codes that aren't
 * part of the QWicks schema). `details` carries the original
 * payload untouched so callers that need more context can read it.
 */
export type QWicksErrorCode =
  | 'validation_error'
  | 'unauthorized'
  | 'forbidden'
  | 'not_found'
...
export type RuntimeErrorCode = QWicksErrorCode | LegacyMainGuardCode
...
const KNOWN_QWICKS_CODES: ReadonlySet<QWicksErrorCode> = new Set<QWicksErrorCode>([
  'validation_error',
  'unauthorized',
  'forbidden',
  'not_found',
...
function normalizeCode(value: unknown): RuntimeErrorCode {
  ...
  if ((KNOWN_QWICKS_CODES as Set<string>).has(value)) return value as QWicksErrorCode
...
/**
 * Parse a QWicks runtime error body. Falls back to the supplied
...
export function isKnownQWicksErrorCode(value: unknown): value is QWicksErrorCode {
  return typeof value === 'string' && (KNOWN_QWICKS_CODES as Set<string>).has(value)
}
