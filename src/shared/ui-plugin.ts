...
/** 形象槽位:缺失的槽位回退默认 QWicks 美术(允许"半皮肤") */
...
/**
 * 与内置模式、DOM 属性值保留字互斥。
 * 注意:'iqwicks' 不在保留字里 —— 内置的 iQWicks 模式本身就是一个预装 UI 插件
 * (见 src/main/ui-plugin-bundled.ts),id 为 'iqwicks' 时额外点亮
 * data-iqwicks-mode 的手工 CSS 机制。
 */
const UI_PLUGIN_RESERVED_IDS = new Set(['default', 'qwicks', 'on', 'off', 'none'])

/** 预装示例插件(iQWicks)的 id:激活时会同时启用 data-iqwicks-mode 手工动画机制 */
export const UI_PLUGIN_BUNDLED_IQWICKS_ID = 'iqwicks'
