...
describe('QWicks agent CLI commands', () => {
  let dataDir = ''

  beforeEach(async () => {
    dataDir = await mkdtemp(join(tmpdir(), 'qwicks-cli-'))
  })
  ...
  it('splits explicit commands and keeps legacy serve flags compatible', () => {
    expect(splitQWicksCliCommand(['run', 'hello'])).toEqual({ command: 'run', args: ['hello'] })
    expect(splitQWicksCliCommand(['--port', '9999'])).toEqual({
      command: 'serve',
      args: ['--port', '9999']
    })
    expect(splitQWicksCliCommand(['nope']).error).toMatch(/unknown command/)
  })

  it('lists tools from qwicks exec with JSON output', async () => {
    const c = capture()
  ...
  it('invokes a direct tool through qwicks exec', async () => {
    const c = capture()
    const code = await runAgentCommand('exec', [
      '--data-dir',
      dataDir,
  ...
  it('lists dynamic runtime tools from qwicks exec', async () => {
  ...
  it('shares config loading between serve and agent commands', async () => {
    const configPath = join(dataDir, 'qwicks.config.json')
