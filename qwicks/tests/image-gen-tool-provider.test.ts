...
function imageGenConfig(overrides: Record<string, unknown> = {}) {
  return QWicksCapabilitiesConfig.parse({
    imageGen: {
      enabled: true,
...
function attachmentStore(rootDir: string, overrides: Record<string, unknown> = {}) {
  return new FileAttachmentStore({
    rootDir,
    config: QWicksCapabilitiesConfig.parse({ attachments: { enabled: true, ...overrides } }).attachments,
...
describe('Image gen tool provider', () => {
  beforeEach(async () => {
    workspace = await mkdtemp(join(tmpdir(), 'qwicks-imagegen-'))
  })
