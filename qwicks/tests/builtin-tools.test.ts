...
describe('QWicks built-in tools', () => {
  let workspace: string
  let host: LocalToolHost

  beforeEach(async () => {
    workspace = await mkdtemp(join(tmpdir(), 'qwicks-tools-'))
  ...
  it('writes, reads, edits, and searches workspace files', async () => {
    ...
    const editOutput = await executeTool(host, workspace, 'edit', {
      path: 'notes/demo.txt',
      edits: [
        { oldText: 'hello world', newText: 'hello qwicks' },
    ...
    expect(editedDisk).toContain('hello qwicks')
    expect(editedDisk).toContain('done')
    expect(String(editOutput.diff)).toContain('+2 hello qwicks')
