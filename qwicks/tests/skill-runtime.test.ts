...
import { QWicksCapabilitiesConfig } from '../src/contracts/capabilities.js'
...
describe('SkillRuntime', () => {
  ...
  beforeEach(async () => {
    root = await mkdtemp(join(tmpdir(), 'qwicks-skills-'))
  })
  ...
  it('discovers a skill package symlinked into a root (e.g. cc switch)', async (ctx) => {
    ...
    const realDir = await mkdtemp(join(tmpdir(), 'qwicks-skill-real-'))
    try {
