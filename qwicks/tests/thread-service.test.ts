...
import { DEFAULT_QWICKS_MODEL } from '../src/config/qwicks-config.js'
...
describe('ThreadService todos', () => {
  it('syncs plan checklists, patches linked checkboxes, and preserves removed tasks', async () => {
    const workspace = await mkdtemp(join(tmpdir(), 'qwicks-todos-'))
    try {
      const relativePath = '.qwickssdd/plan/demo.md'
...
describe('ThreadService.resumeSession', () => {
  it('uses the QWicks default model when resuming item-only legacy sessions', async () => {
    const { service, sessionStore } = buildService()
    ...
    expect(result.thread.model).toBe(DEFAULT_QWICKS_MODEL)
  })
})
