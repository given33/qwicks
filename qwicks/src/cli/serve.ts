...
import {
  qwicksConfigPathForDataDir,
  readQWicksConfigFile,
  readOptionalQWicksConfigFile,
  type LoadedQWicksConfig
} from '../config/qwicks-config.js'

/**
 * Parse the `qwicks serve` command line into validated options.
 *
  ...
  const merged: ServeOptions = {
    ...
    host:
      typeof raw.host === 'string'
        ? raw.host
        : env.QWICKS_HOST ?? configServe.host ?? DEFAULT_SERVE_OPTIONS.host,
    port:
    ...
    endpointFormat:
      typeof raw['endpoint-format'] === 'string'
        ...
        : typeof raw.endpointFormat === 'string'
          ...
          : env.QWICKS_ENDPOINT_FORMAT as ServeOptions['endpointFormat'] | undefined ??
            configServe.endpointFormat ??
            DEFAULT_SERVE_OPTIONS.endpointFormat,
    model:
      typeof raw.model === 'string'
        ? raw.model
        : env.QWICKS_MODEL ?? configServe.model ?? DEFAULT_SERVE_OPTIONS.model,
    approvalPolicy:
 ...
 * main process when QWicks is started programmatically.
 */
...
export const SERVE_USAGE = `qwicks serve [options]
...
function loadServeConfig(
  ...
  env: Record<string, string | undefined>
): LoadedQWicksConfig | null {
  const explicitConfigPath =
    stringFlag(raw, 'config') ??
    stringFlag(raw, 'config-file') ??
    env.QWICKS_CONFIG
  if (explicitConfigPath) {
    return readQWicksConfigFile(explicitConfigPath)
  }
  const dataDir = dataDirFromRawOrEnv(raw, env)
  return readOptionalQWicksConfigFile(qwicksConfigPathForDataDir(dataDir))
}
