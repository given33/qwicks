...
import { createQWicksServeRuntime } from '../server/runtime-factory.js'
...
export const QWICKS_CLI_USAGE = `qwicks <command> [options]
...
Common options:
  ...
  --data-dir <path>          Root directory for QWicks data
...
export type QWicksCliCommand = 'serve' | 'run' | 'chat' | 'exec' | 'help'

export function splitQWicksCliCommand(argv: readonly string[]): {
  command: QWicksCliCommand
  args: string[]
  error?: string
} {
  const first = argv[0]
...
export async function runAgentCommand(
  command: Exclude<QWicksCliCommand, 'serve' | 'help'>,
...
async function runOneShot(argv: readonly string[], io: CliIo): Promise<number> {
  ...
  if (!parsed.ok) return writeParseError(parsed, io, 'qwicks run')
  const prompt = stringFlag(argv, ['prompt', 'p']) ?? positionals(argv).join(' ').trim()
  if (!prompt) {
    io.stderr.write('qwicks run: missing prompt\n')
  ...
  try {
    ...
    io.stderr.write(`qwicks run: ${errorMessage(error)}\n`)
    return ServeExitCode.runtime
  } finally {
    await shutdownRuntime(runtime, io, 'qwicks run')
  }
}

async function runChat(argv: readonly string[], io: CliIo): Promise<number> {
  const parsed = parseSharedOptions(argv, io)
  if (!parsed.ok) return writeParseError(parsed, io, 'qwicks chat')
  let runtime: ServerRuntime | undefined
  try {
    runtime = await createRuntime(parsed.options, io)
    const thread = await runtime.threadService.create({
    ...
    io.stderr.write(`qwicks chat: ${errorMessage(error)}\n`)
    return ServeExitCode.runtime
  } finally {
    await shutdownRuntime(runtime, io, 'qwicks chat')
  }
}
...
async function runExec(argv: readonly string[], io: CliIo): Promise<number> {
  const parsed = parseSharedOptions(argv, io)
  if (!parsed.ok) return writeParseError(parsed, io, 'qwicks exec')
  let runtime: ServerRuntime | undefined
  try {
    runtime = await createRuntime(parsed.options, io)
  } catch (error) {
    io.stderr.write(`qwicks exec: ${errorMessage(error)}\n`)
    return ServeExitCode.runtime
  }
  ...
  try {
    ...
    if (!toolName) {
      io.stderr.write('qwicks exec: missing tool name (use --list-tools to inspect tools)\n')
      return ServeExitCode.usage
    }
    const argsText = stringFlag(argv, ['args']) ?? '{}'
    const args = parseJsonObject(argsText)
    if (!args.ok) {
      io.stderr.write(`qwicks exec: ${args.message}\n`)
    ...
    await shutdownRuntime(runtime, io, 'qwicks exec')
  }
}
...
function parseSharedOptions(argv: readonly string[], io: CliIo): SharedOptionsResult {
  ...
  return {
    ...
    workspace: stringFlag(argv, ['workspace']) ?? io.env?.QWICKS_WORKSPACE ?? io.cwd?.() ?? process.cwd(),
    json: hasFlag(argv, 'json')
  }
}

function createRuntime(options: ServeOptions, io: CliIo): Promise<ServerRuntime> {
  return io.createRuntime ? io.createRuntime(options) : createQWicksServeRuntime(options)
}
