...
import {
  QWICKS_CLI_USAGE,
  runAgentCommand,
  splitQWicksCliCommand
} from './agent-cli.js'
import { startQWicksServe, type QWicksServeHandle } from '../server/runtime-factory.js'

export const QWICKS_READY_PREFIX = 'QWICKS_READY '
 ...
 */
function installServeCrashHandlers(getHandle: () => QWicksServeHandle | null): void {
  let crashing = false
  const crash = (kind: string, error: unknown): void => {
    if (crashing) return
    crashing = true
    const detail = error instanceof Error ? (error.stack ?? error.message) : String(error)
    process.stderr.write(`qwicks serve: ${kind}: ${detail}\n`)
 ...
 * still has the exact same QWICKS_READY handshake behavior.
 */
async function serveMain(argv: readonly string[]): Promise<number> {
  ...
  if (!parsed.ok) {
    process.stderr.write(`qwicks serve: ${parsed.message}\n`)
    if (parsed.issues) {
      process.stderr.write(`${JSON.stringify(parsed.issues, null, 2)}\n`)
    }
    return parsed.exitCode
  }
  let handle: QWicksServeHandle | null = null
  installServeCrashHandlers(() => handle)
  const server = await startQWicksServe(parsed.options)
  handle = server
  ...
  const startupInfo = {
    ...
    pid: info.pid,
    message: `qwicks runtime listening on http://${server.host}:${server.port}`
  }
  process.stdout.write(`${QWICKS_READY_PREFIX}${JSON.stringify(startupInfo)}\n`)
...
/**
 * When the GUI launches qwicks without `ELECTRON_RUN_AS_NODE` (the host
...
export async function main(argv: readonly string[]): Promise<number> {
  ...
  const command = splitQWicksCliCommand(argv)
  if (command.command === 'help') {
    if (command.error) {
      process.stderr.write(`qwicks: ${command.error}\n`)
      process.stderr.write(QWICKS_CLI_USAGE)
      return ServeExitCode.usage
    }
    process.stdout.write(QWICKS_CLI_USAGE)
    return ServeExitCode.ok
  }
...
main(process.argv.slice(2)).then(
  ...
  (error) => {
    process.stderr.write(`qwicks serve: ${String(error)}\n`)
