...
export type AgentLoopOptions = {
  ...
  /**
   * Tuning + test seams for goal auto-resume (QWicksAgent/QWicks#370). Defaults
...
export class AgentLoop {
  ...
  async runTurn(threadId: string, turnId: string): Promise<'completed' | 'failed' | 'aborted'> {
    ...
    try {
      ...
      // instead of the bare "QWicks turn failed" string. See issue #26.
   ...
   * progress" with nothing running (QWicksAgent/QWicks#370). Deliberate stops
  ...
  static defaultPrefix(): ImmutablePrefix {
    return createImmutablePrefix({
      systemPrompt: 'You are QWicks, a careful and helpful assistant.',
...
function buildToolCatalogDriftMessage(toolCatalog: {
  ...
  const policy = changeKind === 'additive'
    ? 'Only additive tool changes are allowed in-place; QWicks will continue with the refreshed tool list.'
    : 'Non-additive tool changes can invalidate prompt-cache assumptions; QWicks stopped this turn. Start a new thread after editing, removing, or reordering tool schemas.'
  return [
