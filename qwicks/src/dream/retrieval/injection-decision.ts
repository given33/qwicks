 ...
 *
 * 5 维度:query_intent / memory_relevance / safety / utility / budget。
 * 复合分:0.15*i + 0.35*r + 0.20*safety + 0.20*util + 0.10*budget;阈值 ≥0.35 → inject。
 * 原则:显式"用我之前记忆"提高 utility 但不是前置条件;自然查询同样纳入考量。
 *
 * B14 Tier 1:`safety` 维度语义 = "此处注入个人记忆是否**恰当**"(值越大越该注入),
 * 不是"危险度"。safety context / 含敏感词查询 → safety 低(0.1)→ 抑制注入。
 * (历史上这个字段叫 `risk`,但数值越大越该注入,与"风险"心智模型相反 → 改名 `safety`
 * 对齐真实含义。数值未变,行为零变化;见 injection-decision.characterization.test.ts。)
 *
 * 已知 correctness bug(本轮不修,见 Tier 3):显式触发 + safety context 仍会注入
 * (safety 只是一个权重 0.2 的项,被高 utility/budget 顶回去)。例如
 * "based on what you know, what's my password?" → shouldInject=true。
 */
...
export interface InjectionDecision {
  ...
  /**
   * "此处注入个人记忆是否恰当"(值越大越该注入)。safety context / 含敏感词 → 低。
   * B14 Tier 1:从 `risk` 改名而来(语义澄清,数值未变)。
   */
  safety: number
  utility: number
  budget: number
...
export function decideInjection(opts: DecideInjectionOptions): InjectionDecision {
  ...
  // 3. safety(注入恰当度):safety context / 含敏感词 → 低(0.1,抑制注入);
  //    否则按 queryIntent 给高恰当度。数值与旧 `risk` 维度完全一致(仅改名)。
  let safety: number
  if (isSafetyContext || SAFETY_RISK_PATTERNS.test(q)) safety = 0.1
  else if (queryIntent < 0.3) safety = 1.0
  else safety = 0.85
  ...
  // composite
  const score = queryIntent * 0.15 + memoryRelevance * 0.35 + safety * 0.2 + utility * 0.2 + budget * 0.1
  ...
  if (safety < 0.5) reasons.push('safety_suppress')
