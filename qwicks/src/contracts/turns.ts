...
export const GuiPlanContextSchema = z.object({
  ...
  relativePath: z
    ...
    .refine(isGuiPlanRelativePath, {
      message: 'relativePath must be a direct Markdown file under .qwickssdd/plan'
    }),
...
export const StartTurnRequest = z.object({
   ...
   * mode QWicks advertises `create_plan` for the whole conversation.
   */
  mode: TurnModeSchema.optional(),
  attachments: z
    .array(
  ...
  workspaceCheckpointId: z.string().min(1).optional(),
  /**
   * Optional GUI plan context. When set, QWicks advertises the
